...
local Destroying = OC:NewPackage("Destroying")
...
local Future = OC.Include("Util.Future")
...
function Destroying.OnInitialize()
	...
	private.myDestroyThread = Threading.New("MyDESTROY", private.MyDestroyThread)
	Threading.SetCallback(private.myDestroyThread, private.MyDestroyThreadDone)
	...
	private.destroyFuture:SetScript("OnCleanup", function()
		...
		private.myDestroyThreadRunning = false
		Threading.Kill(private.destroyThread)
		Threading.Kill(private.myDestroyThread)
	end)
...
function Destroying.StartMyDestroy(query)
	--if  private.destroyFuture:GetState() ~= "RESET" then
	--	return nil
	--end
	private.destroyFuture:Start()
	--if  private.destroyFuture:GetState() ~= "STARTED" then
	--	return nil
	--end
	private.myDestroyThreadRunning = true
	Threading.Start(private.myDestroyThread, query)
	-- we need the thread to run now so send it a sync message
	Threading.SendSyncMessage(private.myDestroyThread, START_MESSAGE)
	return private.destroyFuture
end
...
function private.MyDestroyThread( query)
	-- We get sent a sync message so we run right away
	assert(Threading.ReceiveMessage() == START_MESSAGE)

	local startCount = query:Count()
	--local event = Threading.ReceiveMessage()
	local timeout = GetTime() + 2
	while startCount == query:Count() do
		--event = Threading.ReceiveMessage()
		--print(event)
		if GetTime() > timeout then
			return false
...
function private.SendEventToThread(event)
	if private.destroyThreadRunning then
		Threading.SendMessage(private.destroyThread, event)
		return
	elseif  private.myDestroyThreadRunning then
		Threading.SendMessage(private.myDestroyThread, event)
		return
	end
end
...
function private.MyDestroyThreadDone(result)
	private.myDestroyThreadRunning = false
	private.destroyFuture:Done(result)
end
...
function private.UpdateBagDB()
	...
	if Environment.IsWrathClassic() then
		private.disenchantSkillLevel = OC.Crafting.PlayerProfessions.GetProfessionSkill(UnitName("player"), GetSpellInfo(7411))
		private.jewelcraftSkillLevel = OC.Crafting.PlayerProfessions.GetProfessionSkill(UnitName("player"), GetSpellInfo(28897))
		private.inscriptionSkillLevel = OC.Crafting.PlayerProfessions.GetProfessionSkill(UnitName("player"), GetSpellInfo(45357))
	end
...
function private.IsDestroyable(itemString)
	if  ItemString.GetBase(itemString)~="i:193497"  then
		return false,nil
	end
