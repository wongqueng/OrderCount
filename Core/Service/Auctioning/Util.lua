...
local Util = OC.Auctioning:NewPackage("Util")
...
function Util.GetPrice(key, operation, itemString)
	...
	if not private.priceCache[cacheKey] then
		...
		local minValue, maxValue = OC.Operations.Auctioning.GetMinMaxValues(key)
		...
		for _, subRow in ipairs(subRows) do
			...
			if not Util.IsFiltered(itemString, operationSettings, itemBuyout, quantity, timeLeft) then
				...
				if itemBuyout == lowestItemBuyout then
					...
					temp.isWhitelist = OC.db.factionrealm.auctioningOptions.whitelist[strlower(ownerStr)] and true or false
					...
					if not subRow:HasOwners() and next(OC.db.factionrealm.auctioningOptions.whitelist) then
