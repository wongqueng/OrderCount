...
local PostScan = OC.Auctioning:NewPackage("PostScan")
...
local SlotId = OC.Include("Util.SlotId")
...
function PostScan.OnInitialize()
	...
	private.groupsQuery = OC.Groups.CreateQuery()
		:SetUpdateCallback(private.OnGroupsOperationsChanged)
	private.operationsQuery = OC.Operations.CreateQuery()
		:SetUpdateCallback(private.OnGroupsOperationsChanged)
end
...
function PostScan.DoProcess()
	...
	if result then
		...
		if postRow:GetField("numProcessed") + 1 == postRow:GetField("numStacks") then
			...
			OC.Auctioning.Log.UpdateRowByIndex(auctionId, "state", "POSTED")
		end
	end
...
function PostScan.HandleConfirm(success, canRetry)
	...
	if not confirmRow then
		-- we may have posted something outside of OC
		return
	end
...
function PostScan.ChangePostDetail(field, value)
	...
	elseif field == "buyout" or field == "itemBuyout" then
		...
		OC.Auctioning.Log.UpdateRowByIndex(postRow:GetField("auctionId"), "buyout", itemBuyout)
...
function private.UpdateOperationDB()
	...
	for _, itemString in query:Iterator() do
		local firstOperation = OC.Operations.GetFirstOperationByItem("Auctioning", itemString)
...
function private.ScanThread(auctionScan, scanContext)
	...
	-- record this search
	OC.Auctioning.SavedSearches.RecordSearch(scanContext, scanContext.isItems and "postItems" or "postGroups")
...
function private.IsOperationValid(itemString, num, operationName, operationSettings, noLog)
	...
	if not postCap then
		...
		if not noLog then
			OC.Auctioning.Log.AddEntry(itemString, operationName, "invalidItemGroup", "", 0, math.huge)
		end
		return nil
	elseif postCap == 0 then
		-- posting is disabled, so ignore this operation
		if not noLog then
			OC.Auctioning.Log.AddEntry(itemString, operationName, "postDisabled", "", 0, math.huge)
		end
		return nil
	end
	...
	if Environment.HasFeature(Environment.FEATURES.AH_STACKS) then
		...
		stackSize = OC.Auctioning.Util.GetPrice("stackSize", operationSettings, itemString)
	...
	local keepQuantity = OC.Auctioning.Util.GetPrice("keepQuantity", operationSettings, itemString)
	...
	if num < minPostQuantity then
		...
		if not noLog then
			OC.Auctioning.Log.AddEntry(itemString, operationName, "postNotEnough", "", 0, math.huge)
		end
		return nil
	end
	...
	if maxExpires > 0 then
		...
		if numExpires and numExpires > maxExpires then
			...
			if not noLog then
				OC.Auctioning.Log.AddEntry(itemString, operationName, "postMaxExpires", "", 0, math.huge)
			end
			return nil
		end
	end
...
function private.QueryIsBrowseDoneForItem(query, itemString)
	...
	for _, _, operationSettings in OC.Operations.GroupOperationIterator("Auctioning", groupPath) do
		if isFilterDone then
			...
			for _, subRow in query:ItemSubRowIterator(itemString) do
				...
				if not OC.Auctioning.Util.IsFiltered(itemString, operationSettings, itemBuyout, quantity, timeLeft) then
			...
			for _, operationName, operationSettings in OC.Operations.GroupOperationIterator("Auctioning", groupPath) do
				if private.IsOperationValid(itemString, numHave, operationName, operationSettings, true) then
					...
					if operationNumHave > 0 then
						...
						OC.Auctioning.Log.AddEntry(itemString, operationName, reason, seller, itemBuyout or 0, auctionId)
					end
				end
			end
			assert(numHave >= 0)
...
function private.GeneratePosts(itemString, operationName, operationSettings, numHave, subRows)
	...
	local postCap = OC.Auctioning.Util.GetPrice("postCap", operationSettings, itemString)
	if Environment.HasFeature(Environment.FEATURES.AH_STACKS) then
		local stackSize = OC.Auctioning.Util.GetPrice("stackSize", operationSettings, itemString)
		local maxStackSize = ItemInfo.GetMaxStack(itemString)
	...
	elseif lowestAuction.buyout - undercut < minPrice then
		...
		if resetPrice then
			...
			activeAuctions = OC.Auctioning.Util.GetPlayerAuctionCount(subRows, itemString, operationSettings, floor(bid), buyout, perAuction)
	...
	elseif lowestAuction.isPlayer or (lowestAuction.isWhitelist and OC.db.global.auctioningOptions.matchWhitelist) then
		-- we (or a whitelisted play we should match) are lowest, so match the current price and post as many as we can
		activeAuctions = OC.Auctioning.Util.GetPlayerAuctionCount(subRows, itemString, operationSettings, lowestAuction.bid, lowestAuction.buyout, perAuction)
...
function private.GetPostBagSlot(itemString, quantity)
	...
	if not bagItemString or OC.Groups.TranslateItemString(bagItemString) ~= itemString then
