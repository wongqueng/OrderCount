...
local General = OC.Tooltip:NewPackage("General")
...
local Inventory = OC.Include("Service.Inventory")
...
function General.OnInitialize()
	...
	local tooltipInfo = OC.Tooltip.CreateInfo()
		:SetHeadings(L["OC General Info"])
	...
	OC.Tooltip.Register(tooltipInfo)
end
...
function private.PopulateGroupLine(tooltip, itemString)
	...
	if itemString == ItemString.GetPlaceholder() then
		...
		groupPath = OC.Groups.GetPathByItem(itemString)
		if groupPath == OC.CONST.ROOT_GROUP_PATH then
			groupPath = nil
		else
			itemInGroup = OC.Groups.IsItemInGroup(itemString)
		end
	end
	if groupPath then
		local leftText = nil
		if itemInGroup then
			leftText = GROUP
		elseif ItemString.ParseLevel(OC.Groups.TranslateItemString(itemString)) then
...
function private.PopulateOperationLine(tooltip, itemString, moduleName)
	...
	if itemString == ItemString.GetPlaceholder() then
		...
		for _, operationName in OC.Operations.GroupOperationIterator(moduleName, groupPath) do
			tinsert(operations, operationName)
		end
	end
	if #operations > 0 then
		tooltip:AddLine(format(#operations == 1 and L["%s operation"] or L["%s operations"], OC.Operations.GetLocalizedName(moduleName)), tooltip:ApplyValueColor(table.concat(operations, ", ")))
	end
	TempTable.Release(operations)
end
...
function private.PopulateFullDestroyLines(tooltip, itemString)
	...
	local value, method = OC.Crafting.GetConversionsValue(itemString, private.settings.destroyValueSource)
	if not value then
		return nil, nil
	end
	...
	if method == Conversions.METHOD.DISENCHANT then
		...
		for targetItemString, amountOfMats, matRate, minAmount, maxAmount, targetQuality, sourceQuality in Conversions.TargetItemsByMethodIterator(itemString, method) do
			local matValue = CustomPrice.GetSourcePrice(targetItemString, private.settings.destroyValueSource) or OC.Crafting.GetConversionsValue(targetItemString, private.settings.destroyValueSource) or 0
			if matValue > 0 then
...
function private.PopulateSimpleDestroyLines(tooltip, itemString)
	...
	if itemString == ItemString.GetPlaceholder() then
		...
		value, method = OC.Crafting.GetConversionsValue(itemString, private.settings.destroyValueSource)
	end
	if not value then
		return nil, nil
	end
