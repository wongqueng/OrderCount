...
local AuctionDB = OC.Tooltip:NewPackage("AuctionDB")
...
function AuctionDB.OnInitialize()
	local tooltipInfo = OC.Tooltip.CreateInfo()
		:SetHeadings(L["OC AuctionDB"], private.PopulateRightText)
...
function private.PopulateRightText(tooltip, itemString)
	...
	if itemString == ItemString.GetPlaceholder() then
		...
		lastScan = OC.AuctionDB.GetAppDataUpdateTimes()
		numAuctions = OC.AuctionDB.GetRealmItemData(itemString, "numAuctions") or 0
	end
	if lastScan > 0 then
...
function private.GetItemData(itemString, key)
	if strmatch(key, "^region") then
		return OC.AuctionDB.GetRegionItemData(itemString, key)
	else
		return OC.AuctionDB.GetRealmItemData(itemString, key)
	end
end
