...
local AccountingSync = OC.Accounting:NewPackage("Sync")
...
function private.GetPlayerHash(player)
	...
	OC.Accounting.Transactions.PrepareSyncHashes(player)
	Sync.CallRPC("ACCOUNTING_GET_PLAYER_HASH", player, private.RPCGetPlayerHashResultHandler)
end

function private.RPCGetPlayerHash()
	return OC.Accounting.Transactions.GetSyncHash(Wow.GetCharacterName())
end
...
function private.RPCGetPlayerHashResultHandler(success, player, hash)
	...
	local currentHash = OC.Accounting.Transactions.GetSyncHash(player)
...
function private.RPCGetPlayerChunks()
	return OC.Accounting.Transactions.GetSyncHashByDay(Wow.GetCharacterName())
end
...
function private.RPCGetPlayerChunksResultHandler(success, player, chunks)
	...
	local currentChunks = OC.Accounting.Transactions.GetSyncHashByDay(player)
	...
	for day in pairs(currentChunks) do
		if not chunks[day] then
			...
			OC.Accounting.Transactions.RemovePlayerDay(player, day)
		end
	end
...
function private.RPCGetData(day)
	...
	OC.Accounting.Transactions.GetSyncData(Wow.GetCharacterName(), day, private.dataTemp)
...
function private.RPCGetPlayerDataResultHandler(success, player, day, data)
	...
	OC.Accounting.Transactions.HandleSyncedData(player, day, data)
