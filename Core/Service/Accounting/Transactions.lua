...
local Transactions = OC.Accounting:NewPackage("Transactions")
...
function Transactions.RemoveOldData(days)
	...
	OC.Accounting.Sync.OnTransactionsChanged()
	return numRecords
end
...
function private.LoadFromSettings()
	...
	for _, realm in ipairs(realms) do
		...
		if isLimited then
			Log.PrintfUser(L["Only the last 6 months of Accounting purchases and sales data for %s was loaded. Consider clearing old Accounting data from the OC settings on that realm."], realm)
		end
