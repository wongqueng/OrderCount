...
local Mail = OC.Accounting:NewPackage("Mail")
...
local InventoryInfo = OC.Include("Service.InventoryInfo")
local AuctionTracking = OC.Include("Service.AuctionTracking")
local BagTracking = OC.Include("Service.BagTracking")
local TooltipScanning = OC.Include("Service.TooltipScanning")
local private = {
	hooks = {},
	sellersTimer = nil,
	rescanTimer = nil,
...
function private.ScanCollectedMail(oFunc, attempt, index, subIndex)
	...
	if invoiceType == "seller" and buyer and buyer ~= "" then -- AH Sales
		...
		if private.CanLootMailIndex(index, (bid - ahcut)) then
			if itemString then
				...
				OC.Accounting.Transactions.InsertAuctionSale(itemString, quantity, copper, buyer, saleTime)
			else
	...
	elseif invoiceType == "buyer" and buyer and buyer ~= "" then -- AH Buys
		...
		if itemString and private.CanLootMailIndex(index, 0) then
			...
			OC.Accounting.Transactions.InsertAuctionBuy(itemString, quantity, copper, buyer, buyTime)
			success = true
		else
		...
		if itemString and sender then
			...
			if total ~= 0 and not ignore and private.CanLootMailIndex(index, codAmount) then
				...
				for _ = 1, stacks do
					...
					OC.Accounting.Transactions.InsertCODBuy(itemString, stackSize, copper, sender, buyTime)
	...
	elseif money > 0 and invoiceType ~= "seller" and not strfind(subject, OUTBID_MATCH_TEXT) then
		...
		if sender and private.CanLootMailIndex(index, money) then
			if str and strfind(str, "OC$") then -- payment for a COD the player sent
				...
				if itemString then
					...
					for _ = 1, stacks do
						...
						OC.Accounting.Transactions.InsertCODSale(itemString, stackSize, copper, sender, saleTime)
						qty = qty - stackSize
						if qty <= 0 then
							break
						end
				...
				OC.Accounting.Money.InsertMoneyTransferIncome(money, sender, saleTime)
			end
			success = true
		end
	elseif strfind(subject, EXPIRED_MATCH_TEXT) then -- expired auction
		...
		if private.CanLootMailIndex(index, 0) and itemString and quantity then
			OC.Accounting.Auctions.InsertExpire(itemString, quantity, expiredTime)
			success = true
		end
	elseif strfind(subject, CANCELLED_MATCH_TEXT) then -- cancelled auction
		...
		local itemString = ItemString.Get(link)
		if private.CanLootMailIndex(index, 0) and itemString and quantity then
			OC.Accounting.Auctions.InsertCancel(itemString, quantity, cancelledTime)
			success = true
		end
	end
...
function private.CheckSendMail(destination, currentSubject, ...)
	...
	if moneyAmount > 0 then
		...
		OC.Accounting.Money.InsertMoneyTransferExpense(moneyAmount, destination)
		mailCost = mailCost - moneyAmount
	end
	OC.Accounting.Money.InsertPostageExpense(mailCost, destination)

	if not ignore then
		private.hooks.SendMail(destination, subject .. " (" .. total .. ") OC", ...)
	else
