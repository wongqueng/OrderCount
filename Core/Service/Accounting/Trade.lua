...
local Trade = OC.Accounting:NewPackage("Trade")
...
function private.OnChatMsg(_, msg)
	...
	if msg == LE_GAME_ERR_TRADE_COMPLETE and private.tradeInfo then
		...
		if private.settings.autoTrackTrades then
			...
			if not StaticPopupDialogs["OCAccountingOnTrade"] then
				StaticPopupDialogs["OCAccountingOnTrade"] = {
					button1 = YES,
					button2 = NO,
					timeout = 0,
					whileDead = true,
			...
			StaticPopupDialogs["OCAccountingOnTrade"].text = format(L["OC detected that you just traded %s to %s in return for %s. Would you like Accounting to store a record of this trade?"], gaveText, insertInfo.name, gotText)
			Wow.ShowStaticPopupDialog("OCAccountingOnTrade")
		end
	end
end
...
function private.DoInsert(info)
	if info.type == "sale" then
		OC.Accounting.Transactions.InsertTradeSale(info.itemString, info.count, info.price, info.name)
	elseif info.type == "buy" then
		OC.Accounting.Transactions.InsertTradeBuy(info.itemString, info.count, info.price, info.name)
	else
