...
local AuctionDB = OC:NewPackage("AuctionDB")
...
function AuctionDB.OnEnable()
	local realmData, realmScanStat, realmHistorical, regionCommodity, regionStat, regionHistorical, regionSale = OC.AppHelper.GetAuctionDBData()
	if realmData then
	...
	if not private.realmUpdateTime then
		Log.PrintfUser(L["OC doesn't currently have any AuctionDB pricing data for your realm. We recommend you download the OC Desktop Application from %s to automatically update your AuctionDB data (and auto-backup your OC settings)."], Log.ColorUserAccentText("https://tradeskillmaster.com"))
	end
...
function private.GetBaseItemHelper(itemString)
	...
	if quality and quality >= 2 and itemLevel and itemLevel >= OC.CONST.MIN_BONUS_ID_ITEM_LEVEL and (classId == Enum.ItemClass.Weapon or classId == Enum.ItemClass.Armor) then
