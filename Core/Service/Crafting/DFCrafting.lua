...
local DFCrafting = OC.Crafting:NewPackage("DFCrafting")
local Quality = OC.Include("Service.ProfessionHelpers.Quality")
...
function DFCrafting.GetOptionalMats(craftString, mats, optionalMats)
	if not OC.Crafting.IsQualityCraft(craftString) then
		return false, nil
	end
	local recipeDifficulty, recipeQuality, recipeMaxQuality, inspirationAmount, inspirationChance = OC.Crafting.GetQualityInfo(craftString)
...
function DFCrafting.GetExpectedSalvageResult(method, sourceQuality)
	...
	assert(craftString)
	local baseRecipeDifficulty, baseRecipeQuality = OC.Crafting.GetQualityInfo(craftString)
