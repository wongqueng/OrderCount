...
local CraftingSync = OC.Crafting:NewPackage("Sync")
...
function private.RPCGetCrafts(profession)
	...
	local player = Wow.GetCharacterName()
	local query = OC.Crafting.CreateRawCraftsQuery()
	...
	for _, craftString, itemString in query:Iterator() do
		...
		local baseRecipeDifficulty, baseRecipeQuality, maxRecipeQuality, inspirationAmount, inspirationChance = OC.Crafting.GetQualityInfo(craftString, player)
...
function private.RPCGetCraftsResultHandler(success, player, profession, craftStrings)
	...
	if next(toRemove) then
		OC.Crafting.RemovePlayerSpells(player, toRemove)
	end
	...
	for craftString, info in pairs(craftStrings) do
		if OC.Crafting.HasCraftString(craftString) then
			...
			if type(info) == "string" then
				...
				OC.Crafting.CreateOrUpdatePlayer(craftString, player, tonumber(baseRecipeDifficulty), tonumber(baseRecipeQuality), tonumber(maxRecipeQuality), tonumber(inspirationAmount), tonumber(inspirationChance))
			else
				hash = info
				OC.Crafting.CreateOrUpdatePlayer(craftString, player)
			end
			-- Check if the hash changed in which case we need to re-sync the craft
			if hash ~= private.GetCraftHash(craftString, player, OC.Crafting.GetItemString(craftString)) then
...
function private.RPCGetCraftInfoResultHandler(success, player, profession, info)
	...
	for i, craftString in ipairs(info.craftStrings) do
		OC.Crafting.CreateOrUpdate(craftString, info.itemStrings[i], profession, info.names[i], info.numResults[i], player, info.hasCDs[i] and true or false, info.baseRecipeDifficulties[i], info.baseRecipeQualities[i], info.maxRecipeQualities[i], info.inspirationAmounts[i], info.inspirationChances[i])
		...
		OC.Crafting.SetMats(craftString, info.mats[i])
	end
	OC.Crafting.SetSpellDBQueryUpdatesPaused(false)
