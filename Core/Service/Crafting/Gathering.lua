...
local Gathering = OC.Crafting:NewPackage("Gathering")
...
function Gathering.OnEnable()
	private.db = Database.NewSchema("GATHERING_MATS")
		...
		:Commit()
	private.queuedCraftsUpdateQuery = OC.Crafting.CreateQueuedCraftsQuery()
...
function private.UpdateCrafterList()
	local query = OC.Crafting.CreateQueuedCraftsQuery()
...
function private.UpdateDB()
	...
	for _, recipeString, numQueued in query:Iterator() do
		...
		for _, itemString, quantity in OC.Crafting.MatIterator(craftString) do
		...
		for _, _, itemId in RecipeString.OptionalMatIterator(recipeString) do
			...
			local quantity = OC.Crafting.GetOptionalMatQuantity(craftString, itemId)
	...
	while #matQueue > 0 do
		...
		for _, source in ipairs(private.settings.sources) do
			...
			if isCraftSource then
				...
				local craftString = OC.Crafting.GetMostProfitableCraftStringByItem(itemString, crafter, true)
				if craftString then
					for _, matItemString in OC.Crafting.MatIterator(craftString) do
			...
			if not ignoreSource then
				...
				if numNeed == 0 then
					if isCraftSource then
						...
						local craftString = OC.Crafting.GetMostProfitableCraftStringByItem(itemString, crafter, true)
						assert(craftString)
						local numToCraft = ceil(prevNumNeed / OC.Crafting.GetNumResult(craftString))
						for _, intMatItemString, intMatQuantity in OC.Crafting.MatIterator(craftString) do
...
function private.ProcessSource(itemString, numNeed, source, sourceList)
	...
	elseif source == "alt" then
		...
		for factionrealm, isConnected in OC.db:GetConnectedRealmIterator("factionrealm") do
	...
	elseif source == "craftProfit" or source == "craftNoProfit" then
		local craftString, maxProfit, lowestCraftingCost = OC.Crafting.GetMostProfitableCraftStringByItem(itemString, crafter, true)
