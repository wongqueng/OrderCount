...
local Cost = OC.Crafting:NewPackage("Cost")
...
function Cost.GetCraftedItemValue(itemString)
	local hasCraftPriceMethod, craftPrice = OC.Operations.Crafting.GetCraftedItemValue(itemString)
...
function Cost.GetLowestCostByItem(itemString, optionalMats, qualityMats)
	...
	for _, craftString, hasCD, profession in OC.Crafting.GetCraftStringByItem(levelItemString) do
...
function private.GetOptionalMats(itemString, resultTbl)
	...
	if not isAbs then
		local levelItemString = ItemString.ToLevel(itemString)
		for _, craftString in OC.Crafting.GetCraftStringByItem(levelItemString) do
			local level = CraftString.GetLevel(craftString)
...
function private.GetCraftingCostHelper(craftString, recipeString, optionalMats, qualityMats)
	...
	OC.Crafting.GetMatsAsTable(craftString, mats)
	local chance = 1
	...
	elseif OC.Crafting.IsQualityCraft(craftString) then
		local canCraft, inspirationChance = OC.Crafting.DFCrafting.GetOptionalMats(craftString, mats, qualityMats)
		if canCraft then
	...
	if not private.currentMatProfession then
		private.currentMatProfession = OC.Crafting.GetProfession(craftString)
	...
	cost = Math.Round(cost / (OC.Crafting.GetNumResult(craftString) * chance))
