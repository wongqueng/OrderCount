...
local PlayerProfessions = OC.Crafting:NewPackage("PlayerProfessions")
...
function private.PlayerProfessionsSkillUpdate()
	if Environment.IsRetail() then
		...
		for i, id in pairs(professionIds) do -- needs to be pairs since there might be holes
			...
			if not OC.UI.CraftingUI.IsProfessionIgnored(name) then -- exclude ignored professions
		...
		for i = offset + 1, offset + numSpells do
			...
			if name and subName and (ProfessionInfo.IsSubNameClassic(strtrim(subName, " ")) or name == ProfessionInfo.GetName("Smelting") or name == ProfessionInfo.GetName("Poisons") or name == LEATHERWORKING_ES or name == TAILORING_ES or name == ENGINEERING_FR or name == FIRST_AID_FR) and not OC.UI.CraftingUI.IsProfessionIgnored(name) then
				...
				if level and maxLevel and not OC.UI.CraftingUI.IsProfessionIgnored(name) then -- exclude ignored professions
	...
	for _, character in OC.db:FactionrealmCharacterIterator() do
		local playerProfessions = OC.db:Get("sync", OC.db:GetSyncScopeKeyByCharacter(character), "internalData", "playerProfessions")
...
function private.PlayerProfessionsThread()
	...
	for _, craftString in OC.Crafting.CraftStringIterator() do
	...
	for _, craftString in ipairs(craftStrings) do
		...
		for _, player in OC.Crafting.PlayerIterator(craftString) do
			-- check if the player still exists and still has this profession
			local playerProfessions = OC.db:Get("sync", OC.db:GetSyncScopeKeyByCharacter(player), "internalData", "playerProfessions")
			if not playerProfessions or not playerProfessions[OC.Crafting.GetProfession(craftString)] then
				playersToRemove[player] = true
			end
		end
		local stillExists = true
		if next(playersToRemove) then
			stillExists = OC.Crafting.RemoveCraftPlayers(craftString, playersToRemove)
		end
		TempTable.Release(playersToRemove)
		if stillExists then
			for _, itemString in OC.Crafting.MatIterator(craftString) do
