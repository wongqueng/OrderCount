...
local Crafting = OC:NewPackage("Crafting")
...
local DisenchantInfo = OC.Include("Data.DisenchantInfo")
...
local Conversions = OC.Include("Service.Conversions")
...
function Crafting.CreateCraftsQuery()
	return private.spellDB:NewQuery()
		:AggregateJoinSummed(OC.Crafting.Queue.GetDBForJoin(), "craftString", "num")
		...
		:VirtualField("craftingCost", "number", OC.Crafting.Cost.GetCraftingCostByCraftString, "craftString", Math.GetNan())
		:VirtualField("itemValue", "number", OC.Crafting.Cost.GetCraftedItemValue, "itemString", Math.GetNan())
		:VirtualField("profit", "number", OC.Crafting.Cost.GetProfitByCraftString, "craftString", Math.GetNan())
...
function Crafting.CreateQueueQuery()
	return OC.Crafting.Queue.CreateQuery()
...
function Crafting.GetMostProfitableCraftStringByItem(itemString, playerFilter, noCD)
	...
	for _, craftString, hasCD in Crafting.GetCraftStringByItem(itemString) do
		if (not playerFilter or playerFilter == "" or Crafting.HasPlayer(craftString, playerFilter)) and (not noCD or not hasCD) then
			local craftingCost, _, profit = OC.Crafting.Cost.GetCostsByCraftString(craftString)
...
function Crafting.RestockHelp(link)
	...
	if not itemString then
		Log.PrintUser(L["No item specified. Usage: /oc restock_help [ITEM_LINK]"])
		return
	end
...
function Crafting.GetConversionsValue(itemString, customPrice, method)
	...
	for targetItemString, rate, _, _, _, targetQuality, sourceQuality, _, targetItemMethod in Conversions.TargetItemsByMethodIterator(itemString, method) do
		...
		local quality = sourceQuality and OC.Crafting.DFCrafting.GetExpectedSalvageResult(method, sourceQuality)
...
function private.SaleRateVirtualField(itemString)
	return OC.AuctionDB.GetRegionItemData(itemString, "regionSalePercent") or Math.GetNan()
end
...
function private.GetRestockHelpMessage(itemString)
	...
	if not groupPath then
		return L["This item is not in a OC group."]
	end

	-- check that there's a crafting operation applied
	if not OC.Operations.Crafting.HasOperation(itemString) then
		return format(L["There is no Crafting operation applied to this item's OC group (%s)."], GroupPath.Format(groupPath))
	end
	...
	if not OC.Crafting.CanCraftItem(itemString) then
		return L["You don't know how to craft this item."]
	end

	-- check the restock quantity
	local neededQuantity = OC.Operations.Crafting.GetRestockQuantity(itemString, private.GetTotalQuantity(itemString))
	...
	local cost, craftString = OC.Crafting.Cost.GetLowestCostByItem(itemString)
	local numResult = craftString and OC.Crafting.GetNumResult(craftString)
	...
	if hasMinProfit then
		local craftedValue = OC.Crafting.Cost.GetCraftedItemValue(itemString)
