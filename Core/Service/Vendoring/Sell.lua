...
local Sell = OC.Vendoring:NewPackage("Sell")
...
function Sell.OnInitialize()
	...
	for itemString in pairs(OC.db.global.userData.vendoringIgnore) do
...
function Sell.IgnoreItemPermanent(itemString)
	assert(not OC.db.global.userData.vendoringIgnore[itemString])
	OC.db.global.userData.vendoringIgnore[itemString] = true
...
function Sell.ForgetIgnoreItemPermanent(itemString)
	assert(OC.db.global.userData.vendoringIgnore[itemString])
	OC.db.global.userData.vendoringIgnore[itemString] = nil
...
function private.UpdatePotentialValueDB()
	...
	for _, itemString in query:Iterator() do
		local value = CustomPrice.GetValue(OC.db.global.vendoringOptions.qsMarketValue, itemString)
		if value then
