...
local Groups = OC.Vendoring:NewPackage("Groups")
...
function private.BuyThread(groups)
	...
	local query = OC.Vendoring.Buy.CreateMerchantQuery()
		:InnerJoin(OC.Groups.GetItemDBForJoin(), "itemString")
	...
	for _, itemString, groupPath, numAvailable in query:Iterator() do
		if groups[groupPath] then
			local _, operationSettings = OC.Operations.GetFirstOperationByItem("Vendoring", itemString)
	...
	for _, itemString in ipairs(itemsToBuy) do
		...
		OC.Vendoring.Buy.BuyItem(itemString, numToBuy)
...
function private.SellThread(groups)
	...
	for _, groupPath in ipairs(groups) do
		if groupPath ~= OC.CONST.ROOT_GROUP_PATH then
			wipe(operationsTemp)
			for _, operationName, operationSettings in OC.Operations.GroupOperationIterator("Vendoring", groupPath) do
			...
			for _, operationName in ipairs(operationsTemp) do
				for _, itemString in OC.Groups.ItemIterator(groupPath) do
					totalValue = totalValue + private.SellItemThreaded(itemString, OC.Operations.GetSettings("Vendoring", operationName))
				end
			end
		end
	end
	...
	if OC.db.global.vendoringOptions.displayMoneyCollected then
...
function private.SellItemThreaded(itemString, operationSettings)
	...
	-- check the expires
	if operationSettings.sellAfterExpired > 0 and OC.Accounting.Auctions.GetNumExpiresSinceSale(itemString) < operationSettings.sellAfterExpired then
		return 0
	end
