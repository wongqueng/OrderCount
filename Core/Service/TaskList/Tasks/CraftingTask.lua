...
local CraftingTask = OC.Include("LibOCClass").DefineClass("CraftingTask", OC.TaskList.Task)
...
OC.TaskList.CraftingTask = CraftingTask
local private = {
...
function CraftingTask.Acquire(self, doneHandler, category, profession)
	...
	for _, _, prof, skillId in OC.Crafting.PlayerProfessions.Iterator() do
...
function CraftingTask.OnMouseDown(self)
	if self._buttonText == L["CRAFT"] then
		...
		OC.Crafting.ProfessionUtil.PrepareToCraft(craftString, nil, quantity)
	end
end
...
function CraftingTask.OnButtonClick(self)
	if self._buttonText == L["CRAFT"] then
		...
		private.pendingItemString = OC.Crafting.GetItemString(craftString)
		local numCrafted = OC.Crafting.ProfessionUtil.Craft(craftString, spellId, quantity, true, nil, private.CraftCompleteCallback)
...
function CraftingTask._UpdateState(self)
	self:_SortCraftStrings()
	if OC.Crafting.ProfessionUtil.GetNumCraftableFromDB(self._craftStrings[1]) == 0 then
...
function CraftingTask._SortCraftStrings(self)
	...
	for _, craftString in ipairs(self._craftStrings) do
		private.numCraftableTemp[craftString] = OC.Crafting.ProfessionUtil.GetNumCraftableFromDB(craftString)
	end
...
function private.SubTaskIterator(self, index)
	...
	return index, OC.Crafting.GetName(craftString).." ("..self._craftQuantity[craftString]..")"
end
