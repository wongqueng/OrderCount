...
local SendMailTask = OC.Include("LibOCClass").DefineClass("SendMailTask", OC.TaskList.ItemTask)
local L = OC.Include("Locale").GetTable()
local DefaultUI = OC.Include("Service.DefaultUI")
OC.TaskList.SendMailTask = SendMailTask
local private = {
...
function SendMailTask.OnButtonClick(self)
	...
	OC.Mailing.Send.StartSending(private.SendCallback, self._target, "", "", 0, self:GetItems())
	self:_UpdateState()
