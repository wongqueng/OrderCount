...
local Expirations = OC.TaskList:NewPackage("Expirations")
...
local private = {
	mailTaskPool = ObjectPool.New("EXPIRING_MAIL_TASK", OC.TaskList.ExpiringMailTask, 0),
	auctionTaskPool = ObjectPool.New("EXPIRED_AUCTION_TASK", OC.TaskList.ExpiredAuctionTask, 0),
	activeTasks = {},
	expiringMailTasks = {},
	expiredAuctionTasks = {},
...
function private.PopulateTasks()
	...
	-- expiring mails
	for k, v in pairs(OC.db.factionrealm.internalData.expiringMail) do
		...
		if expiration <= DAYS_LEFT_LIMIT * -1 then
			OC.db.factionrealm.internalData.expiringMail[PLAYER_NAME] = nil
		else
	...
	-- expired auctions
	for k, v in pairs(OC.db.factionrealm.internalData.expiringAuction) do
