...
local Groups = OC:NewPackage("Groups") ---@class OC.Groups
...
function Groups.RebuildDatabase()
	...
	for groupPath, groupInfo in pairs(private.settings.groups) do
		if type(groupPath) == "string" and GroupPath.IsValid(groupPath) then
			...
			for key in pairs(groupInfo) do
				if OC.Operations.ModuleExists(key) then
					-- This is a set of module operations
					local operations = groupInfo[key]
					while #operations > OC.Operations.GetMaxNumber(key) do
					...
					for i = #operations, 1, -1 do
						if type(operations[i]) ~= "string" or operations[i] == "" or not OC.Operations.Exists(key, operations[i]) then
	...
	if not private.settings.groups[GroupPath.GetRoot()] then
		...
		for _, moduleName in OC.Operations.ModuleIterator() do
			assert(OC.Operations.Exists(moduleName, "#Default"))
...
function Groups.AppendOperation(groupPath, moduleName, operationName, skipRebuild)
	assert(OC.Operations.Exists(moduleName, operationName))
	local groupOperations = private.settings.groups[groupPath][moduleName]
	assert(groupOperations.override and #groupOperations < OC.Operations.GetMaxNumber(moduleName))
...
function private.RebuildDB()
	...
	for groupPath in pairs(private.settings.groups) do
		...
		local hasAuctioningOperation = OC.Operations.GroupHasAnyOperation("Auctioning", groupPath)
		local hasCraftingOperation = OC.Operations.GroupHasAnyOperation("Crafting", groupPath)
		local hasMailingOperation = OC.Operations.GroupHasAnyOperation("Mailing", groupPath)
		local hasShoppingOperation = OC.Operations.GroupHasAnyOperation("Shopping", groupPath)
		local hasSniperOperation = OC.Operations.GroupHasAnyOperation("Sniper", groupPath)
		local hasVendoringOperation = OC.Operations.GroupHasAnyOperation("Vendoring", groupPath)
		local hasWarehousingOperation = OC.Operations.GroupHasAnyOperation("Warehousing", groupPath)
