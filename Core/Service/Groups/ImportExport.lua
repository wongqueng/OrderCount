...
local ImportExport = OC.Groups:NewPackage("ImportExport")
...
local SERIALIZE_OPTIONS = {
	...
	filter = function(tbl, key, value)
		return not private.isOperationSettingsTable[tbl] or not OC.Operations.IsCommonKey(key)
	end,
}
...
function ImportExport.GenerateExport(exportGroupPath, includeSubGroups, excludeOperations, excludeCustomSources)
	assert(exportGroupPath ~= OC.CONST.ROOT_GROUP_PATH)
	...
	for _, groupPath in OC.Groups.GroupIterator() do
		...
		elseif groupPath == exportGroupPath then
			relGroupPath = OC.CONST.ROOT_GROUP_PATH
		end
			...
			if not excludeOperations then
				...
				for moduleName in pairs(EXPORT_OPERATION_MODULES) do
					groupOperations[relGroupPath][moduleName] = {
						...
						override = OC.Groups.HasOperationOverride(groupPath, moduleName) or groupPath == exportGroupPath or nil,
					}
					for _, operationName, operationSettings in OC.Operations.GroupOperationIterator(moduleName, groupPath) do
...
function ImportExport.GetImportTotals()
	...
	for itemString, groupPath in pairs(private.importContext.items) do
		if not private.importContext.filteredGroups[groupPath] then
			if OC.Groups.IsItemInGroup(itemString) then
	...
	for moduleName, moduleOperations in pairs(private.importContext.operations) do
		...
		for operationName, operationSettings in pairs(moduleOperations) do
			if usedOperations[operationName] then
				...
				if OC.Operations.Exists(moduleName, operationName) then
...
function ImportExport.CommitImport(moveExistingItems, includeOperations, replaceOperations)
	...
	if includeOperations and next(private.importContext.operations) then
		...
		OC.Operations.BulkCreateFromImport(private.importContext.operations, replaceOperations)
	end
	...
	-- create the groups
	local numItems = OC.Groups.BulkCreateFromImport(private.importContext.groupName, private.importContext.items, private.importContext.groups, private.importContext.groupOperations, moveExistingItems)
...
function private.DecodeNewImport(str)
	...
	elseif version ~= VERSION then
		...
		return false
	elseif type(groupName) ~= "string" or groupName == "" or strmatch(groupName, OC.CONST.GROUP_SEP) then
...
function private.DecodeGroupExportHelper(str)
	...
	local relativePath = OC.CONST.ROOT_GROUP_PATH
...
function private.ValidateOperationsTable(operations, strict)
	...
	for moduleName, moduleOperations in pairs(operations) do
		...
		for operationName, operationSettings in pairs(moduleOperations) do
			if type(operationName) ~= "string" or not OC.Operations.IsValidName(operationName) then
			...
			if OC.Operations.SanitizeSettings(moduleName, operationName, operationSettings, true, true) then
	...
	for groupPath, groupsOperationsTable in pairs(groupOperations) do
		...
		for moduleName, moduleOperations in pairs(groupsOperationsTable) do
			...
			elseif moduleOperations.override ~= nil and moduleOperations.override ~= true then
				...
				return false
			elseif groupPath == OC.CONST.ROOT_GROUP_PATH and not moduleOperations.override then
				if strict then
			...
			if numOperations > OC.Operations.GetMaxNumber(moduleName) then
...
function private.DedupImportGroupName(groupName)
	if OC.Groups.Exists(groupName) then
		local num = 1
		while OC.Groups.Exists(groupName.." "..num) do
			num = num + 1
		end
...
function private.IsValidOperationModule(moduleName)
	if type(moduleName) ~= "string" then
		Log.Err("Invalid moduleName (%s)", tostring(moduleName))
		return false
	elseif not OC.Operations.ModuleExists(moduleName) then
		Log.Err("Invalid moduleName (%s)", tostring(moduleName))
		return false
...
function private.UpdateTopLevelGroup(topLevelGroup, items, groups, groupOperations)
	...
	groups[OC.CONST.ROOT_GROUP_PATH] = nil
	for groupPath in pairs(groups) do
	...
	if groupOperations[OC.CONST.ROOT_GROUP_PATH] then
		for _, moduleOperations in pairs(groupOperations[OC.CONST.ROOT_GROUP_PATH]) do
			moduleOperations.override = true
		end
	end
end
