...
function OrderLog.OnDisable()
	...
	for _, item, orderType, client, player, commission, quantity, timestamp ,saveTime in query:Iterator() do
		-- add the save time
		local storetime=saveTime ~= 0 and saveTime or time()
		tinsert(orderRecordSaveTimes, storetime)
		-- add to our list of CSV lines
		CSV.EncodeAddRowDataRaw(orderRecordEncodeContext, item, orderType, client, player, commission, quantity, timestamp)
	end
	query:Release()
