...
local Operations = OC:NewPackage("Operations")
...
function Operations.OnInitialize()
	private.db = Database.NewSchema("OPERATIONS")
		...
		:AddIndex("moduleName")
		:Commit()
	if OC.db.global.coreOptions.globalOperations then
		private.operations = OC.db.global.userData.operations
	else
		private.operations = OC.db.profile.userData.operations
	end
	private.RebuildDB()
	private.shouldCreateDefaultOperations = not OC.db.profile.internalData.createdDefaultOperations
	OC.db.profile.internalData.createdDefaultOperations = true
	OC.db:RegisterCallback("OnProfileUpdated", private.OnProfileUpdated)
end
...
function Operations.IsValidName(operationName)
	return operationName == strtrim(operationName) and operationName ~= "" and not strmatch(operationName, OC.CONST.OPERATION_SEP)
end
...
function Operations.Rename(moduleName, oldName, newName)
	...
	OC.Groups.OperationRenamed(moduleName, oldName, newName)
	private.RebuildDB()
end
...
function Operations.Delete(moduleName, operationName)
	...
	private.RemoveDeadRelationships(moduleName)
	OC.Groups.RemoveOperationFromAllGroups(moduleName, operationName)
	OC.Groups.RebuildDB()
	private.RebuildDB()
end
...
function Operations.GetFirstOperationByItem(moduleName, itemString)
	...
	for _, operationName in OC.Groups.OperationIterator(groupPath, moduleName) do
...
function Operations.IsStoredGlobally()
	return OC.db.global.coreOptions.globalOperations
end

function Operations.SetStoredGlobally(storeGlobally)
	OC.db.global.coreOptions.globalOperations = storeGlobally
	...
	if storeGlobally then
		...
		OC.db.global.userData.operations = CopyTable(OC.db.profile.userData.operations)
		-- clear out old operations
		local originalProfile = OC.db:GetCurrentProfile()
		for _, profile in OC.db:ScopeKeyIterator("profile") do
			OC.db:SetProfile(profile)
			OC.db.profile.userData.operations = nil
		end
		OC.db:SetProfile(originalProfile)
	else
		-- move global to all profiles
		local originalProfile = OC.db:GetCurrentProfile()
		for _, profile in OC.db:ScopeKeyIterator("profile") do
			OC.db:SetProfile(profile)
			OC.db.profile.userData.operations = CopyTable(OC.db.global.userData.operations)
		end
		OC.db:SetProfile(originalProfile)
		-- clear out old operations
		OC.db.global.userData.operations = nil
	end
...
function Operations.ReplaceProfileOperations(newOperations)
	for k, v in pairs(newOperations) do
		OC.db.profile.userData.operations[k] = v
	end
end
...
function Operations.GroupIterator(moduleName, filterOperationName, overrideOnly)
	...
	if Operations.GroupHasOperation(moduleName, OC.CONST.ROOT_GROUP_PATH, filterOperationName) then
		tinsert(result, OC.CONST.ROOT_GROUP_PATH)
	end
	-- need to filter out the groups without operations
	for _, groupPath in OC.Groups.GroupIterator() do
		if (not overrideOnly or OC.Groups.HasOperationOverride(groupPath, moduleName)) and Operations.GroupHasOperation(moduleName, groupPath, filterOperationName) then
