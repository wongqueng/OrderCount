...
local Auctioning = OC.Operations:NewPackage("Auctioning")
...
function Auctioning.OnInitialize()
	...
	OC.Operations.Register("Auctioning", L["Auctioning"], OPERATION_INFO, 20, private.GetOperationInfo, private.OperationSanitize)
end
...
function private.GetOperationValueHelper(itemString, key)
	...
	local operationName, operationSettings = OC.Operations.GetFirstOperationByItem("Auctioning", itemString)
	if not operationName then
		return
	end
	return OC.Auctioning.Util.GetPrice(key, operationSettings, origItemString)
end
...
function private.GetOperationInfo(operationSettings)
	...
	if operationSettings.postCap == 0 then
		...
		if Environment.HasFeature(Environment.FEATURES.AH_STACKS) then
			tinsert(parts, format(L["Posting %d stack(s) of %d for %s hours."], operationSettings.postCap, operationSettings.stackSize, strmatch(OC.CONST.AUCTION_DURATIONS[operationSettings.duration], "%d+")))
		else
			tinsert(parts, format(L["Posting %d items for %s hours."], operationSettings.postCap, strmatch(OC.CONST.AUCTION_DURATIONS[operationSettings.duration], "%d+")))
		end
	end
