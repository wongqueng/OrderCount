...
local Mailing = OC.Operations:NewPackage("Mailing")
...
function Mailing.OnInitialize()
	OC.Operations.Register("Mailing", L["Mailing"], OPERATION_INFO, 50, private.GetOperationInfo)
end


function Mailing.TargetShortfallGetNumToBags(itemString, numHave)
	local totalNumToSend = 0
	for _, _, operationSettings in OC.Operations.GroupOperationIterator("Mailing", OC.Groups.GetPathByItem(itemString)) do
