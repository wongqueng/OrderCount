...
local FilterSearch = OC.Shopping:NewPackage("FilterSearch")
...
local FilterSearchContext = OC.Include("LibOCClass").DefineClass("FilterSearchContext", OC.Shopping.ShoppingSearchContext)
local private = {
	settings = nil,
	scanThreadId = nil,
	itemFilter = nil,
...
function private.ScanThread(auctionScan, filterStr)
	...
	if Environment.IsRetail() and filterStr == "" then
		...
		if not private.isSpecial then
			OC.Shopping.SavedSearches.RecordFilterSearch(filterStr)
		end
	end
