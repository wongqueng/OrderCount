...
local Groups = OC.UI.VendoringUI:NewPackage("Groups")
...
function Groups.OnInitialize()
	...
	private.FSMCreate()
	OC.UI.VendoringUI.RegisterTopLevelPage(L["Groups"], private.GetFrame)
end
...
function private.GetFrame()
	...
	return UIElements.New("Frame", "buy")
		...
		:AddChild(UIElements.New("ApplicationGroupTree", "groupTree")
			...
			:SetQuery(OC.Groups.CreateQuery(), "Vendoring")
		...
		:AddState(FSM.NewState("ST_BUSY")
			:SetOnEnter(function(context, operation)
				...
				if operation == "BUY" then
					OC.Vendoring.Groups.BuyGroups(groups, private.FSMSellCallback)
				elseif operation == "SELL" then
					OC.Vendoring.Groups.SellGroups(groups, private.FSMSellCallback)
				else
			...
			:SetOnExit(function(context)
				...
				OC.Vendoring.Groups.StopBuySell()
			end)
			:AddTransition("ST_FRAME_OPEN")
			:AddTransition("ST_FRAME_CLOSED")
