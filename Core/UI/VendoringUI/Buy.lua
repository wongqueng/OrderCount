...
local Buy = OC.UI.VendoringUI:NewPackage("Buy")
...
function Buy.OnInitialize()
	...
	OC.UI.VendoringUI.RegisterTopLevelPage(L["Buy"], private.GetFrame)
end
	...
	local frame = UIElements.New("Frame", "buy")
		...
		:AddChild(UIElements.New("Frame", "footer")
			...
			:AddChild(UIElements.New("ActionButton", "repairBtn")
				:SetDisabled(not OC.Vendoring.Buy.NeedsRepair())
				:SetText(L["Repair"])
...
function private.GetCostText(row)
	...
	local color = OC.Vendoring.Buy.GetMaxCanAfford(index) < stackSize and Theme.GetColor("FEEDBACK_RED"):GetTextColorPrefix()
...
function private.RowOnClick(scrollingTable, row, mouseButton)
	if IsShiftKeyDown() then
		...
		local dialogFrame = UIElements.New("Frame", "frame")
			...
			:AddChild(UIElements.New("ActionButton", "purchaseBtn")
				:SetHeight(24)
				:SetText(L["Purchase"])
				:SetDisabled(OC.Vendoring.Buy.GetMaxCanAfford(row:GetField("index")) < 1)
	...
	elseif mouseButton == "RightButton" then
		OC.Vendoring.Buy.BuyItemIndex(row:GetFields("index", "stackSize"))
	end
end
...
function private.InputQtyOnValidationChanged(input)
	...
	if input:IsValid() and tonumber(input:GetValue()) <= OC.Vendoring.Buy.GetMaxCanAfford(row:GetField("index")) then
...
function private.InputQtyOnValueChanged(input)
	...
	input:GetElement("__parent.__parent.cost")
		:Draw()
	if input:IsValid() and value <= OC.Vendoring.Buy.GetMaxCanAfford(row:GetField("index")) then
		input:GetElement("__parent.__parent.purchaseBtn")
			:SetDisabled(false)
			:Draw()
	else
...
function private.MaxBtnOnClick(button)
	...
	local value = max(1, min(OC.Vendoring.Buy.GetMaxCanAfford(row:GetField("index")), ItemInfo.GetMaxStack(row:GetField("itemString")) * CalculateTotalNumberOfFreeBagSlots()))
...
function private.GetAltCostText(row, quantity)
	local index, costItemsText, price, stackSize = row:GetFields("index", "costItemsText", "price", "stackSize")
	local color = OC.Vendoring.Buy.GetMaxCanAfford(index) < quantity and Theme.GetColor("FEEDBACK_RED"):GetTextColorPrefix()
...
function private.GetItemAltCostText(row, quantity)
	...
	if extendedCost then
		...
		for j = 1, numAltCurrencies do
			...
			if OC.Vendoring.Buy.GetMaxCanAfford(index) < quantity then
...
function private.PurchaseBtnOnClick(button)
	...
	OC.Vendoring.Buy.BuyItemIndex(row:GetField("index"), button:GetElement("__parent.qty.input"):GetValue())
	button:GetBaseElement():HideDialog()
end
...
function private.RepairOnClick(button)
	...
	if IsAltKeyDown() then
		if not OC.Vendoring.Buy.CanGuildRepair() then
			Log.PrintfUser(L["Cannot repair from the guild bank!"])
			return
		end
		OC.Vendoring.Buy.DoGuildRepair()
	else
		OC.Vendoring.Buy.DoRepair()
	end
end
