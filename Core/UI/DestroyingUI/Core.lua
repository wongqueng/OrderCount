...
local DestroyingUI = OC.UI:NewPackage("DestroyingUI")
...
local UIManager = OC.Include("UI.UIManager")
...
function DestroyingUI.OnEnable()
	...
	private.manager:SetStateFromPublisher("canCombine", OC.Destroying.CanCombinePublisher())
...
function private.ActionHandler(state, action)
	...
	elseif action == "ACTION_COMBINE_START" then
		...
		local future = OC.Destroying.StartCombine()
	...
	elseif action == "ACTION_DESTROY_START" then
		...
		local future = OC.Destroying.StartDestroy(state.frame:GetElement("content.destroyBtn"), state.frame:GetElement("content.items"):GetSelection())
...
function private.CreateMainFrame(state)
	local frame = UIElements.New("ApplicationFrame", "base")
		...
		:SetContentFrame(UIElements.New("Frame", "content")
			...
			:AddChild(UIElements.NewNamed("SecureMacroActionButton", "destroyBtn", "OCDestroyBtn")
				:SetHeight(26)
