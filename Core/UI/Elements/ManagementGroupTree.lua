...
local ManagementGroupTree = OC.Include("LibOCClass").DefineClass("ManagementGroupTree", OC.UI.GroupTree)
local DragContext = OC.Include("UI.DragContext")
local UIElements = OC.Include("UI.UIElements")
UIElements.Register(ManagementGroupTree)
OC.UI.ManagementGroupTree = ManagementGroupTree
local private = {}
...
function ManagementGroupTree.SetSelectedGroup(self, groupPath, redraw)
	...
	if redraw then
		...
		self._contextTable.collapsed[OC.CONST.ROOT_GROUP_PATH] = nil
...
function private.OnActionIconClick(self, data, iconIndex)
	if iconIndex == 1 then
		...
		if OC.Groups.Exists(newGroupPath) then
			local num = 1
			while OC.Groups.Exists(newGroupPath.." "..num) do
				num = num + 1
			end
			newGroupPath = newGroupPath.." "..num
		end
		OC.Groups.Create(newGroupPath)
	...
	elseif iconIndex == 2 then
		local groupColor = Theme.GetGroupColor(select('#', strsplit(OC.CONST.GROUP_SEP, data)))
...
function private.DeleteConfirmed(self, data)
	OC.Groups.Delete(data)
	Analytics.Action("DELETED_GROUP", data)
	self:SetSelectedGroup(OC.CONST.ROOT_GROUP_PATH, true)
end
...
function private.RowOnDragStop(row)
	...
	OC.Groups.Move(oldPath, newPath)
