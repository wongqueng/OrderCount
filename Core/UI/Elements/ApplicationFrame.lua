...
function ApplicationFrame.Acquire(self)
	self:AddChildNoLayout(UIElements.New("Frame", "titleFrame")
		...
		:AddChild(UIElements.New("Text", "icon")
			:SetMargin(0, 16, 0, 0)
			:SetText("代工助手")
		)
...
function ApplicationFrame.AddPlayerGold(self)
	...
	titleFrame:AddChildBeforeById(titleFrame:HasChildById("switchBtn") and "switchBtn" or "closeBtn", OC.UI.Views.PlayerGoldText.New("playerGold")
		:SetWidth("AUTO")
		:SetMargin(0, 8, 0, 0)
	)
	return self
...
function ApplicationFrame.AddAppStatusIcon(self)
	...
	local appUpdateAge = time() - (OC.AppHelper.GetLastSync() or 0)
	local auctionDBRealmTime, auctionDBRegionTime = OC.AuctionDB.GetAppDataUpdateTimes()
...
function ApplicationFrame.SetContentFrame(self, frame)
	assert(frame:__isa(OC.UI.Frame))
	frame:WipeAnchors()
...
function private.GetAppStatusTooltip()
	...
	local regionRealmName = OC.AppHelper.GetRegion().."-"..GetRealmName()
	...
	tinsert(tooltipLines, format(L["OC Desktop App Status (%s)"], regionRealmName))
