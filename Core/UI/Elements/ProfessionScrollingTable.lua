...
local ProfessionScrollingTable = OC.Include("LibOCClass").DefineClass("ProfessionScrollingTable", OC.UI.ScrollingTable)
local UIElements = OC.Include("UI.UIElements")
UIElements.Register(ProfessionScrollingTable)
OC.UI.ProfessionScrollingTable = ProfessionScrollingTable
local private = {
	activeElements = {},
	categoryInfoCache = {
		parent = {},
...
function private.GetQtyCellText(self, data)
	...
	local num, numAll = OC.Crafting.ProfessionUtil.GetNumCraftable(data)
...
function private.GetSaleRateCellText(self, data)
	local saleRate = self._isCraftString[data] and OC.Crafting.Cost.GetSaleRateByCraftString(data)
...
function private.GetCostsByCraftString(craftString)
	...
	if not private.costsCache.isCached[craftString] then
		local craftingCost, itemValue, profit = OC.Crafting.Cost.GetCostsByCraftString(craftString)
