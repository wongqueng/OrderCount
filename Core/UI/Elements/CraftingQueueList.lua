...
local CraftingQueueList = OC.Include("LibOCClass").DefineClass("CraftingQueueList", OC.UI.ScrollingTable)
local UIElements = OC.Include("UI.UIElements")
UIElements.Register(CraftingQueueList)
OC.UI.CraftingQueueList = CraftingQueueList
local private = {
	categoryOrder = {},
	sortSelf = nil,
	sortProfitCache = {},
...
function CraftingQueueList._UpdateData(self)
	...
	for _, row in self._query:Iterator() do
		...
		if not self._collapsed[rawCategory] then
			...
			self._numCraftableCache[row] = OC.Crafting.ProfessionUtil.GetNumCraftableFromDBRecipeString(recipeString)
			private.sortProfitCache[recipeString] = OC.Crafting.Cost.GetProfitByRecipeString(recipeString)
...
function private.GetItemIcon(self, data)
	...
	local tooltip, texture = OC.Crafting.ProfessionUtil.GetCraftResultTooltipFromRecipeString(data:GetField("recipeString"))
...
function private.GetItemTooltip(self, data)
	...
	local profit = OC.Crafting.Cost.GetProfitByRecipeString(recipeString)
	...
	for _, matItemString, quantity in OC.Crafting.MatIterator(craftString) do
	...
	for _, _, itemId in RecipeString.OptionalMatIterator(recipeString) do
		...
		local numHave = BagTracking.GetCraftingMatQuantity(matItemString)
		local numNeed = OC.Crafting.GetOptionalMatQuantity(craftString, itemId)* numQueued
		local color = Theme.GetColor(numHave >= numNeed and "FEEDBACK_GREEN" or "FEEDBACK_RED")
		tinsert(tooltipLines, color:ColorText(numHave.."/"..numNeed).." - "..(UIUtils.GetDisplayItemName(matItemString) or "?"))
	end
...
function private.OnDeleteIconClick(self, data, iconIndex)
	...
	OC.Crafting.Queue.SetNum(data:GetField("recipeString"), 0)
end
...
function private.DialogOnHide(frame)
	...
	OC.Crafting.Queue.SetNum(self._dialogRecipeString, self._dialogInputValue)
	self._dialogRecipeString = nil
	self._dialogInputValue = nil
	self:Draw()
end
