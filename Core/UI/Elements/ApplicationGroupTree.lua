...
local ApplicationGroupTree = OC.Include("LibOCClass").DefineClass("ApplicationGroupTree", OC.UI.GroupTree)
UIElements.Register(ApplicationGroupTree)
OC.UI.ApplicationGroupTree = ApplicationGroupTree
...
function ApplicationGroupTree._HandleRowClick(self, data, mouseButton)
	...
	for _, groupPath in ipairs(self._allData) do
		if GroupPath.IsChild(groupPath, data) and data ~= OC.CONST.ROOT_GROUP_PATH then
