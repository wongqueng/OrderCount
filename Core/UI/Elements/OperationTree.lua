...
local OperationTree = OC.Include("LibOCClass").DefineClass("OperationTree", OC.UI.ScrollingTable)
...
OC.UI.OperationTree = OperationTree
local private = {}
local DATA_SEP = "\001"
...
function OperationTree._UpdateData(self)
	...
	for _, moduleName in OC.Operations.ModuleIterator() do
		...
		for _, operationName in OC.Operations.OperationIterator(moduleName) do
...
function private.OnActionIconClick(self, data, iconIndex)
	...
	if iconIndex == 1 then
		if operationName then
			-- duplicate
			local num = 1
			while OC.Operations.Exists(moduleName, operationName.." "..num) do
				num = num + 1
			end
