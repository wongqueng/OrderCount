...
-- A group tree is an abstract element which displays OC groups. It is a subclass of the @{ScrollingTable} class.
-- @classmod GroupTree
...
function GroupTree.ExpandAll(self)
	for _, groupPath in ipairs(self._allData) do
		if groupPath ~= OC.CONST.ROOT_GROUP_PATH and self._hasChildrenLookup[groupPath] and self._contextTable.collapsed[groupPath] then
...
function GroupTree.CollapseAll(self)
	for _, groupPath in ipairs(self._allData) do
		if groupPath ~= OC.CONST.ROOT_GROUP_PATH and self._hasChildrenLookup[groupPath] and not self._contextTable.collapsed[groupPath] then
...
function GroupTree._UpdateData(self)
	...
	if self._moduleOperationFilter then
		...
		for _, row in self._query:Iterator() do
			...
			if shouldKeep[groupPath] then
				shouldKeep[OC.CONST.ROOT_GROUP_PATH] = true
				-- add all parent groups to the keep table as well
				local checkPath = GroupPath.GetParent(groupPath)
				while checkPath and checkPath ~= OC.CONST.ROOT_GROUP_PATH do
	...
	for groupPath in pairs(self._contextTable.collapsed) do
		if groupPath == OC.CONST.ROOT_GROUP_PATH or not pathExists[groupPath] or not self._hasChildrenLookup[groupPath] then
	...
	for _, groupPath in ipairs(groups) do
		...
		if self._searchStr ~= "" or not self:_IsGroupHidden(groupPath) then
			...
			if strmatch(strlower(groupName), self._searchStr) and (self._searchStr == "" or groupPath ~= OC.CONST.ROOT_GROUP_PATH) then
...
function GroupTree._IsGroupHidden(self, data)
	if data == OC.CONST.ROOT_GROUP_PATH then
		return false
	elseif self._contextTable.collapsed[OC.CONST.ROOT_GROUP_PATH] then
		return true
	end
	local parent = GroupPath.GetParent(data)
	while parent and parent ~= OC.CONST.ROOT_GROUP_PATH do
...
function GroupTree._HandleRowClick(self, data, mouseButton)
	if mouseButton == "RightButton" and self._searchStr == "" and data ~= OC.CONST.ROOT_GROUP_PATH and self._hasChildrenLookup[data] then
...
function private.GetGroupText(self, data)
	local groupName = data == OC.CONST.ROOT_GROUP_PATH and L["Base Group"] or GroupPath.GetName(data)
	if data ~= OC.CONST.ROOT_GROUP_PATH then
		groupName = Theme.GetGroupColor(select('#', strsplit(OC.CONST.GROUP_SEP, data))):ColorText(groupName)
	end
	return groupName
end
...
function private.GetExpanderState(self, data)
	...
	if data == OC.CONST.ROOT_GROUP_PATH then
		...
		local level = select('#', strsplit(OC.CONST.GROUP_SEP, data))
		indentWidth = (searchIsActive and 0 or (level - 1)) * (TextureAtlas.GetWidth("iconPack.14x14/Caret/Right") + EXPANDER_SPACING)
	end
	return not searchIsActive and data ~= OC.CONST.ROOT_GROUP_PATH and self._hasChildrenLookup[data], not self._contextTable.collapsed[data], nil, indentWidth, EXPANDER_SPACING, true
end
...
function private.RowOnDoubleClick(row, mouseButton)
	...
	assert(self._searchStr == "" and data ~= OC.CONST.ROOT_GROUP_PATH and self._hasChildrenLookup[data])
	self:_SetCollapsed(data, not self._contextTable.collapsed[data])
	self:UpdateData(true)
end
