...
local Gathering = OC.UI.CraftingUI:NewPackage("Gathering")
...
function Gathering.OnInitialize()
	...
	OC.UI.CraftingUI.RegisterTopLevelPage(L["Gathering"], private.GetGatheringFrame)
	OC.Crafting.Gathering.SetContextChangedCallback(private.ContextChangedCallback)
	OC.UI.TaskListUI.RegisterUpdateCallback(private.UpdateButtonState)
end
	...
	local frame = UIElements.New("DividedContainer", "gathering")
		...
		:SetRightChild(UIElements.New("Frame", "mats")
			...
			:AddChild(UIElements.New("QueryScrollingTable", "table")
				...
				:GetScrollingTableInfo()
					...
					:NewColumn("need")
						...
						:Commit()
					:Commit()
				:SetQuery(OC.Crafting.Gathering.CreateQuery()
			...
			:AddChild(UIElements.New("ActionButton", "openTaskListBtn")
				:SetHeight(26)
				:SetMargin(8)
				:SetScript("OnClick", OC.UI.TaskListUI.Toggle)
			)
		)
...
function private.CrafterDropdownOnSelectionChanged(dropdown)
	OC.Crafting.Gathering.SetCrafter(dropdown:GetSelectedItem() or "")
	dropdown:GetElement("__parent.professionDropdown")
		:SetItems(OC.Crafting.Gathering.GetProfessionList())
		:SetSelectedItems(OC.Crafting.Gathering.GetProfessions())
		:Draw()
end
...
function private.ProfessionDropdownOnSelectionChanged(dropdown)
	...
	OC.Crafting.Gathering.SetProfessions(professions)
...
function private.ContextChangedCallback()
	...
	private.frame:GetElement("setup.crafterDropdown")
		:SetItems(OC.Crafting.Gathering.GetCrafterList())
		:SetSelectedItem(OC.Crafting.Gathering.GetCrafter())
		:Draw()
...
function private.UpdateButtonState()
	...
	elseif OC.UI.TaskListUI.IsVisible() then
