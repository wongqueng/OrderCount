...
local CraftingReports = OC.UI.CraftingUI:NewPackage("CraftingReports")
...
function CraftingReports.OnInitialize()
	...
	OC.UI.CraftingUI.RegisterTopLevelPage(L["Reports"], private.GetCraftingReportsFrame)
end
...
function private.GetCraftingReportsFrame()
	...
	if not private.craftsQuery then
		private.craftsQuery = OC.Crafting.CreateCraftsQuery()
	...
	private.matsQuery = private.matsQuery or OC.Crafting.CreateMatItemQuery()
...
function private.GetTabElements(self, path)
	if path == L["Crafts"] then
		...
		for _, player, profession in OC.Crafting.PlayerProfessions.Iterator() do
	...
	elseif path == L["Materials"] then
		...
		for _, _, profession in OC.Crafting.PlayerProfessions.Iterator() do
...
function private.CraftsOnRowClick(scrollingTable, record, mouseButton)
	...
	if mouseButton == "LeftButton" then
		OC.Crafting.Queue.Add(recipeString, 1)
	elseif mouseButton == "RightButton" then
		OC.Crafting.Queue.Remove(recipeString, 1)
	end
end
...
function private.MatsOnRowClick(scrollingTable, row)
	...
	scrollingTable:GetBaseElement():ShowDialogFrame(OC.UI.Views.CustomStringDialog.New(priceStr, L["Material Cost"], private.MatPriceValidateFunc, nil, private.MatPriceDialogOnHide, itemString)
...
function private.FirstOperationVirtualField(itemString)
	return OC.Operations.GetFirstOperationByItem("Crafting", itemString) or ""
end
...
function private.IsCraftableQueryFilter(record)
	return OC.Crafting.ProfessionUtil.GetNumCraftableFromDB(record:GetField("craftString")) > 0
end
...
function private.CraftsMenuClickHandler(scrollingTable, index1, index2)
	if index1 == "CREATE_GROUPS" then
		...
		for _, row in private.craftsQuery:Iterator() do
			...
			if not OC.Groups.Exists(groupPath) then
				OC.Groups.Create(groupPath)
				numCreated = numCreated + 1
			end
			if not OC.Groups.IsItemInGroup(itemString) and not ItemInfo.IsSoulbound(itemString) then
				OC.Groups.SetItemGroup(itemString, groupPath)
				numAdded = numAdded + 1
			end
		end
