...
local Send = OC.UI.MailingUI:NewPackage("Send")
...
function Send.OnInitialize()
	...
	private.FSMCreate()
	OC.UI.MailingUI.RegisterTopLevelPage(L["Send"], private.GetSendFrame)
...
function private.ContactsBtnOnClick(button)
	OC.UI.Util.Contacts.ShowDialog(button, button:GetElement("__parent.input"), private.RecipientOnValueChanged)
end
...
function private.UpdateRecentlyMailed(recipient)
	...
	for k, v in pairs(OC.db.global.mailingOptions.recentlyMailedList) do
		size = size + 1
	...
	if size >= 20 then
		OC.db.global.mailingOptions.recentlyMailedList[oldestName] = nil
	end

	OC.db.global.mailingOptions.recentlyMailedList[recipient] = time()
end
	...
	private.fsm = FSM.New("MAILING_SEND")
		...
		:AddState(FSM.NewState("ST_SENDING_START")
			:SetOnEnter(function(context, keepInfo, recipient, subject, body, money, items)
				...
				OC.Mailing.Send.StartSending(private.FSMSendCallback, recipient, subject, body, money, items)
				UpdateButton(context)
			end)
