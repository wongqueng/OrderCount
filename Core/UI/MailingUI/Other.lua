...
local Groups = OC.UI.MailingUI:NewPackage("Other")
...
function Groups.OnInitialize()
	private.FSMCreate()
	OC.UI.MailingUI.RegisterTopLevelPage(OTHER, private.GetOtherFrame)
end
	...
	local frame = UIElements.New("Frame", "other")
		...
		:AddChild(UIElements.New("Frame", "enchantHeader")
			...
			:AddChild(UIElements.New("Input", "recipient")
				...
				:SetValue(OC.db.factionrealm.internalData.mailDisenchantablesChar)
		...
		:AddChild(UIElements.New("Frame", "goldHeader")
			...
			:AddChild(UIElements.New("Input", "recipient")
				...
				:SetAutoComplete(PlayerInfo.GetConnectedAlts())
				:SetClearButtonEnabled(true)
				:SetValue(OC.db.factionrealm.internalData.mailExcessGoldChar)
			...
			:AddChild(UIElements.New("Input", "limit")
				...
				:SetValue(Money.ToString(OC.db.factionrealm.internalData.mailExcessGoldLimit, nil, "OPT_TRIM"))
...
function private.EchantRecipientOnValueChanged(input)
	...
	if value == OC.db.factionrealm.internalData.mailDisenchantablesChar then
		return
	end
	OC.db.factionrealm.internalData.mailDisenchantablesChar = value
	private.UpdateEnchantButton()
end
...
function private.EnchantSendBtnOnClick(button)
	...
	for _, itemString, quantity in query:Iterator() do
		if ItemInfo.IsDisenchantable(itemString) then
			...
			if quality <= OC.db.global.mailingOptions.deMaxQuality then
				items[itemString] = (items[itemString] or 0) + quantity
			end
		end
	end
	query:Release()
	private.fsm:ProcessEvent("EV_BUTTON_CLICKED", OC.db.factionrealm.internalData.mailDisenchantablesChar, 0, items)
end

function private.GoldRecipientOnValueChanged(input)
	local value = input:GetValue()
	if value == OC.db.factionrealm.internalData.mailExcessGoldChar then
		return
	end
	OC.db.factionrealm.internalData.mailExcessGoldChar = value
	private.UpdateGoldButton()
end

function private.GoldSendBtnOnClick(button)
	local money = private.GetSendMoney()
	private.fsm:ProcessEvent("EV_BUTTON_CLICKED", OC.db.factionrealm.internalData.mailExcessGoldChar, money)
end
...
function private.LimitOnValueChanged(input)
	...
	assert(value)
	if value == OC.db.factionrealm.internalData.mailExcessGoldLimit then
		return
	end
	OC.db.factionrealm.internalData.mailExcessGoldLimit = value
	private.UpdateGoldButton()
end
...
function private.GetSendMoney()
	local money = GetMoney() - 30 - OC.db.factionrealm.internalData.mailExcessGoldLimit
	if money < 0 then
		money = 0
	end
...
function private.UpdateEnchantButton()
	local recipient = OC.db.factionrealm.internalData.mailDisenchantablesChar
...
function private.UpdateGoldButton()
	local recipient = OC.db.factionrealm.internalData.mailExcessGoldChar
	...
	private.fsm = FSM.New("MAILING_GROUPS")
		...
		:AddState(FSM.NewState("ST_SENDING_START")
			:SetOnEnter(function(context, recipient, money, items)
				...
				if money > 0 then
					OC.Mailing.Send.StartSending(private.FSMOthersCallback, recipient, "OC Mailing: Excess Gold", "", money)
					UpdateGold(context)
				elseif items then
					OC.Mailing.Send.StartSending(private.FSMOthersCallback, recipient, "OC Mailing: Disenchantables", "", money, items)
