...
local Inbox = OC.UI.MailingUI:NewPackage("Inbox")
...
function Inbox.OnInitialize()
	...
	private.FSMCreate()
	OC.UI.MailingUI.RegisterTopLevelPage(INBOX, private.GetInboxFrame)
end
...
function private.GetInboxMailsFrame()
	private.inboxQuery = private.inboxQuery or OC.Mailing.Inbox.CreateQuery()
	...
	if Environment.IsRetail() then
		frame:AddChild(UIElements.New("Frame", "bottom")
			...
			:SetPadding(8)
			:SetBackgroundColor("PRIMARY_BG_ALT")
			:AddChild(UIElements.NewNamed("ActionButton", "openAllMail","MyOpenAllMail")
				:SetHeight(26)
...
function private.ReplyOnClick(button)
	local _, _, sender = GetInboxHeaderInfo(private.selectedMail)
	OC.UI.MailingUI.Send.SetSendRecipient(sender)
	OC.UI.MailingUI.SetSelectedTab(L["Send"], true)
end
...
function private.UpdateCountDown(force)
	...
	local nextUpdate = MAIL_REFRESH_TIME - (time() - OC.Mailing.Open.GetLastCheckTime())
	...
	private.fsm = FSM.New("MAILING_INBOX")
		...
		:AddState(FSM.NewState("ST_OPENING_START")
			:SetOnEnter(function(context, autoRefresh, keepMoney, filterText, filterType)
				...
				UpdateButtons(context)
				OC.Mailing.Open.StartOpening(private.FSMOpenCallback, autoRefresh, keepMoney, filterText, filterType)
			end)
