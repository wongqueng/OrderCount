...
function ActionButton:__init(name, isSecure)
	...
	frame:OCSetScript(isSecure and "PostClick" or "OnClick", self:__closure("_HandleClick"))
	frame:OCSetScript("OnMouseDown", self:__closure("_HandleMouseDown"))
	frame:OCSetScript("OnEnter", self:__closure("_HandleFrameEnter"))
	frame:OCSetScript("OnLeave", self:__closure("_HandleFrameLeave"))
...
function ActionButton:Acquire()
	...
	self._state:PublisherForKeyChange("locked")
		:CallMethod(frame, "OCSetHighlightLocked")
	...
	self._state:PublisherForKeyChange("clickCooldownActive")
		...
		:CallMethod(frame, "OCSetOnUpdate")
end
...
function ActionButton:GetDisabled()
