...
function RangeInput:__init()
	...
	frame:OCSetScript("OnMouseDown", self:__closure("_HandleFrameMouseDown"))
	frame:OCSetScript("OnMouseUp", self:__closure("_HandleFrameMouseUp"))
	frame:OCSetOnUpdate(self:__closure("_HandleFrameUpdate"))
	...
	frame.inputLeft:OCSetScript("OnEscapePressed", self:__closure("_HandleInputEscapePressed"))
	frame.inputLeft:OCSetScript("OnEnterPressed", self:__closure("_HandleInputEnterPressed"))
	...
	frame.dash:OCSetFont("BODY_BODY2")
	frame.dash:SetText("-")
	...
	frame.inputRight:OCSetScript("OnEscapePressed", self:__closure("_HandleInputEscapePressed"))
	frame.inputRight:OCSetScript("OnEnterPressed", self:__closure("_HandleInputEnterPressed"))
...
function RangeInput:Acquire()
	...
	frame.barTexture:OCSubscribeColorTexture("FRAME_BG")
	frame.activeBarTexture:OCSubscribeColorTexture("TEXT")

	-- Font
	self._state:PublisherForKeyChange("font")
		:CallMethod(frame.inputLeft, "OCSetFont")
	self._state:PublisherForKeyChange("font")
		:CallMethod(frame.dash, "OCSetFont")
	self._state:PublisherForKeyChange("font")
		:CallMethod(frame.inputRight, "OCSetFont")

	-- Text color
	self._state:PublisherForKeyChange("textColor")
		:CallMethod(frame.inputLeft, "OCSubscribeTextColor")
	self._state:PublisherForKeyChange("textColor")
		:CallMethod(frame.dash, "OCSubscribeTextColor")
	self._state:PublisherForKeyChange("textColor")
		:CallMethod(frame.inputRight, "OCSubscribeTextColor")
...
function RangeInput:Release()
	local frame = self:_GetBaseFrame()
	frame.barTexture:OCCancelAll()
	frame.activeBarTexture:OCCancelAll()
	frame.inputLeft:OCCancelAll()
	frame.dash:OCCancelAll()
	frame.inputRight:OCCancelAll()
	self._minValue = nil
	self._maxValue = nil
	self._dragging = nil
