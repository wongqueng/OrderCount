...
function SyntaxInput:__init()
	...
	frame:OCSetScript("OnUpdate", self:__closure("_HandleFrameUpdate"))
	frame:OCSetScript("OnMouseWheel", self:__closure("_HandleFrameMouseWheel"))
	frame:OCSetScript("OnMouseUp", self:__closure("_HandleFrameMouseUp"))

	self._scrollbar = OC.UI.Scrollbar.Create(frame)
	self._scrollbar:OCSetScript("OnValueChanged", self:__closure("_HandleScrollbarValueChanged"))

	self._backgroundTexture = Rectangle.New(frame, 1)
	self._backgroundTexture:SetCornerRadius(CORNER_RADIUS)

	self._editBox:OCSetFont("BODY_BODY2")
	self._editBox:OCSubscribeTextColor("TEXT")
	...
	self._editBox:OCSetScript("OnEscapePressed", self:__closure("_HandleEscapePressed"))
	self._editBox:OCSetScript("OnKeyDown", self:__closure("_HandleKeyDown"))
	self._editBox:OCSetScript("OnMouseUp", self:__closure("_HandleMouseUp"))
	self._editBox:OCSetScript("OnCursorChanged", self:__closure("_HandleCursorChanged"))
	self._editBox:OCSetScript("OnSizeChanged", self:__closure("_HandleSizeChanged"))
...
function SyntaxInput:Acquire()
	...
	self._editBox:OCSetScript("OnEnterPressed", self:__closure("_HandleEnterPressed"))
	self._editBox:OCSetScript("OnTextChanged", self:__closure("_HandleTextChanged"))
...
function SyntaxInput:Release()
	...
	self._editBox:OCCancelAll()
	self._editBox:OCSetScript("OnEnterPressed", nil)
	self._editBox:OCSetScript("OnTextChanged", nil)
...
function SyntaxInput:Draw()
	...
	self._scrollbar.thumb:SetHeight(OC.UI.Scrollbar.GetLength(self._editBox:GetHeight(), self:_GetDimension("HEIGHT")))
end
...
function SyntaxInput:_HandleFrameMouseWheel(_, direction)
	...
	while parent do
		if parent:__isa(OC.UI.ScrollFrame) then
			parentScroll = parent
			break
		else
