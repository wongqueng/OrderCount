...
function Button:__init(name)
	local frame = UIElements.CreateFrame(self, "Button",name)
...
function Button:Acquire()
	...
	self._state:PublisherForKeyChange("enabled")
		:CallMethod(frame, "OCSetEnabled")

	-- Set whether or not the background is visible
	self._state:PublisherForKeyChange("background")
		:MapToBoolean()
		:CallMethod(frame.backgroundTexture, "OCSetShown")

	-- Set the background texture
	self._state:PublisherForKeyChange("background")
		:IgnoreNil()
		:MapWithFunction(private.BackgroundToTexture)
		:CallMethod(frame.backgroundTexture, "OCSetTextureAndCoord")

	-- Set the background color
	self._state:PublisherForKeyChange("background")
		:IgnoreNil()
		:MapWithFunction(private.BackgroundToColor)
		:CallMethod(frame.backgroundTexture, "OCSubscribeVertexColor")
	...
	self._state:PublisherForKeyChange("background")
		...
		:IgnoreNil()
		:CallMethod(frame.backgroundTexture, "OCSetSize")

	-- Set the background anchors
	self._state:PublisherForKeyChange("background")
		:IgnoreNil()
		:MapWithFunction(private.BackgroundToPoints)
		:CallMethod(frame.backgroundTexture, "OCSetPoints")
	...
	self._state:PublisherForKeyChange("highlightTexture")
		:CallMethod(frame.highlight, "OCSetTextureAndCoord")

	-- Set the text color
	self._state:PublisherForKeyChange("textAndIconColor")
		:CallMethod(frame.text, "OCSubscribeTextColor")

	-- Set whether or not the icon is visible
	self._state:PublisherForKeyChange("iconTexturePack")
		:MapToBoolean()
		:IgnoreDuplicates()
		:CallMethod(frame.icon, "OCSetShown")

	-- Set the icon texture
	self._state:PublisherForKeyChange("iconTexturePack")
		:IgnoreNil()
		:CallMethod(frame.icon, "OCSetTextureAndSize")

	-- Set the icon vertex color
	self._state:PublisherForKeyChange("textAndIconColor")
		:CallMethod(frame.icon, "OCSubscribeVertexColor")
...
function Button:Release()
	...
	frame.backgroundTexture:OCCancelAll()
	frame.highlight:OCCancelAll()
	frame.text:OCCancelAll()
	frame.icon:OCCancelAll()
