...
local ListRow = OC.Include("LibOCClass").DefineClass("ListRow") ---@class ListRow
OC.UI.ListRow = ListRow
...
function ListRow:__init()
	...
	self._frame = frame
	frame:OCSetScript("OnEnter", self:__closure("_FrameOnEnter"))
	frame:OCSetScript("OnLeave", self:__closure("_FrameOnLeave"))
	frame:OCSetScript("OnClick", self:__closure("_FrameOnClick"))
...
function ListRow:Acquire(parentFrame, height, frameEventHandler)
	...
	self._state:PublisherForKeyChange("backgroundColor")
		:CallMethod(frame.background, "OCSubscribeColorTexture")
	...
	self._state:Publisher()
		...
		:CallMethod(frame.highlight, "OCSetShown")

	-- Set the highlight color
	self._state:PublisherForKeyChange("backgroundColor")
		:MapWithFunction(private.BackgroundColorKeyToHighlightColorKey)
		:CallMethod(frame.highlight, "OCSubscribeColorTexture")
end
...
function ListRow:Release()
	...
	frame.background:OCCancelAll()
	frame.highlight:OCCancelAll()
	...
	for _, button in pairs(self._used.buttons) do
		...
		button:OCSetScript("OnEnter", nil)
		button:OCSetScript("OnLeave", nil)
		button:OCSetScript("OnClick", nil)
		button:OCSetScript("OnMouseDown", nil)
		button:OCSetScript("OnMouseUp", nil)
		button:OCSetScript("OnUpdate", nil)
...
function ListRow:SetScript(script, handler)
	self._frame:OCSetScript(script, handler, self)
end
...
function ListRow:EnableDragging(dragStartHandler, dragStopHandler)
	...
	frame:OCSetScript("OnDragStart", dragStartHandler, self)
	frame:OCSetScript("OnDragStop", dragStopHandler, self)
end
