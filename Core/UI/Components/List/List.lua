...
local private = {
	rowPool = ObjectPool.New("LIST_ROWS", OC.UI.ListRow, 1),
}
...
function List:__init()
	...
	self._hScrollFrame:OCSetScript("OnUpdate", self:__closure("_HScrollFrameOnUpdate"))
	self._hScrollFrame:OCSetScript("OnMouseWheel", self:__closure("_FrameOnMouseWheel"))
	...
	self._vScrollFrame:OCSetScript("OnUpdate", self:__closure("_VScrollFrameOnUpdate"))
	self._vScrollFrame:OCSetScript("OnMouseWheel", self:__closure("_FrameOnMouseWheel"))
	...
	self._hScrollbar = OC.UI.Scrollbar.Create(frame, true)
	self._vScrollbar = OC.UI.Scrollbar.Create(frame)
...
function List:Acquire(rowHeight)
	...
	self._state:PublisherForKeyChange("backgroundColor")
		:CallMethod(frame, "OCSubscribeBackdropColor")
	self._state:PublisherForKeyChange("backgroundColor")
	...
	self._vScrollbar:OCSetScript("OnValueChanged", self:__closure("_OnVScrollbarValueChangedNoDraw"))
	-- don't want to cause this element to be drawn for this initial scrollbar change
	self._vScrollbar:SetValue(0)
	self._vScrollbar:OCSetScript("OnValueChanged", self:__closure("_OnVScrollbarValueChanged"))

	self._hScrollbar:OCSetScript("OnValueChanged", self:__closure("_OnHScrollbarValueChangedNoDraw"))
	-- don't want to cause this element to be drawn for this initial scrollbar change
	self._hScrollbar:SetValue(0)
	self._hScrollbar:OCSetScript("OnValueChanged", self:__closure("_OnHScrollbarValueChanged"))
end
...
function List.__protected:_DrawHFrames()
	...
	self._hScrollbar.thumb:SetWidth(OC.UI.Scrollbar.GetLength(self._hContent:GetWidth(), visibleWidth))
...
function List.__protected:_DrawVFrames()
	...
	self._vScrollbar.thumb:SetHeight(OC.UI.Scrollbar.GetLength(totalHeight, visibleHeight))
