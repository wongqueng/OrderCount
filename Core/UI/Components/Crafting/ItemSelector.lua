...
function ItemSelector:__init(button)
	...
	frame:OCSetScript("OnMouseUp", self:__closure("_HandleClick"))
	...
	self._quality:OCSetFont("BODY_BODY1")
	...
	self._quantity:OCSetFont("TABLE_TABLE1_OUTLINE")
	...
	self._addIcon:OCSetTextureAndSize("iconPack.14x14/Add/Default")
...
function ItemSelector:CanSalvage(selection)
	local slotid=private.GetBagSlotId(selection, self._requiredQty)
	if slotid == nil then
		return false
	else
		return true
