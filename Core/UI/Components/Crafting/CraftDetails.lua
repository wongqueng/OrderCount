...
local ProfessionInfo = OC.Include("Data.ProfessionInfo")
...
local cuoxian_types={}
table.insert(cuoxian_types,"i:193050") --破布
table.insert(cuoxian_types,"i:193922") --绿布
local zhakuang_types={}
table.insert(zhakuang_types,"i:190396") --宁铁2
table.insert(zhakuang_types,"i:190394") --宁铁3
table.insert(zhakuang_types,"i:194545") --棱光
--table.insert(zhakuang_types,"i:192880") --崩裂
local baoshichen_types={}
table.insert(baoshichen_types,"i:192850") -- 琥珀2星
table.insert(baoshichen_types,"i:192851")  -- 琥珀3星
table.insert(baoshichen_types,"i:192847")  -- 玛瑙2星
table.insert(baoshichen_types,"i:192848")  -- 玛瑙3星
table.insert(baoshichen_types,"i:192844")
table.insert(baoshichen_types,"i:192845")
table.insert(baoshichen_types,"i:192841")
table.insert(baoshichen_types,"i:192842")
table.insert(baoshichen_types,"i:192838")
table.insert(baoshichen_types,"i:192839")
table.insert(baoshichen_types,"i:192852")
table.insert(baoshichen_types,"i:192853")
table.insert(baoshichen_types,"i:192856")
table.insert(baoshichen_types,"i:192857")
table.insert(baoshichen_types,"i:192859")
table.insert(baoshichen_types,"i:192860")
table.insert(baoshichen_types,"i:192862")
table.insert(baoshichen_types,"i:192863")
table.insert(baoshichen_types,"i:192866")
table.insert(baoshichen_types,"i:192867")
table.insert(baoshichen_types,"i:192868")
...
function CraftDetails:Acquire()
	...
	self:AddChild(UIElements.New("Frame", "content")
		...
		:AddChild(UIElements.New("Frame", "buttons")
			...
			:AddChild(UIElements.New("Frame", "content")
				:SetLayout("HORIZONTAL")
				:SetMargin(0, 0, 6, 6)
				:AddChild(UIElements.NewNamed("ActionButton", "craftBtn","MyCraftBtn")
					:SetWidth(80)
					:SetMargin(0, 6, 0, 0)
					:SetText(L["Craft"])
				...
				:AddChild(UIElements.NewNamed("ActionButton", "craftVellumBtn","MyCraftVellumBtn")
					:SetWidth(144)
					:SetMargin(0, 0, 0, 0)
			...
			:AddChild(UIElements.NewNamed("ActionButton", "FenJie","MyFenJie")
								:SetWidth(230)
								:SetMargin(0, 0, 0, 6)
								:SetText("分解")
								:SetScript("OnClick", self:__closure("DestroyButtonPreClick"))
								)
	...
	private.state=self.state;
	private.query = OC.Destroying.CreateBagQuery()
	...
	local  fenjieBtn=buttons:GetElement("FenJie")
	self._state:PublisherForKeyChange("craftString")
		:MapBooleanEquals("c:376509:q5")
		:CallMethod(fenjieBtn, "SetShown")
	self._state:PublisherForKeyChange("destroyFuture")
		  :MapToBoolean()
		  :MapBooleanWithValues(L["Destroying..."], L["Destroy Next"])
		:CallMethod(fenjieBtn, "SetShown")

	self._state:Publisher()
		  :IgnoreDuplicatesWithKeys("canDestroy",  "destroyFuture")
		  :MapWithFunction(private.StateToDestroyDisabled)
			:CallFunction(self:__closure("_UpdateFenJie"))
			--:CallMethod(fenjieBtn, "SetDisabled")

	self._state:PublisherForKeyChange("destroyFuture")
		 :MapToBoolean()
		:CallMethod(fenjieBtn, "SetPressed")
...
function CraftDetails:Release()
	...
	self._onQueueButtonClick = nil
	self._onCraftButtonMouseDown = nil
	self._onCuoXianEnd = nil
	self._onZhaKuangEnd = nil
	self._onCraftButtonClick = nil
...
function CraftDetails:GetRecipeString()
	return self._state.recipeString
end
...
function CraftDetails:SetState(craftingType, craftingQuantity)
	...
	local zhakuang_end=false
	local cuoxian_end=false
	-- 搓线
	if self._state.craftString == "c:376562" then
		cuoxian_end=true
		for i,v in ipairs(cuoxian_types) do
			local CanSalvage=self:GetElement("content.selection.content.itemSelector")
								 :CanSalvage(v)
			if CanSalvage  then
	...
	-- 做护腕
	if self._state.craftString == "c:376509:q5" then
		self:GetElement("content.buttons.quantity.input")
			:SetValue(9999)
			:Draw()
		private.FenJieCountUpdated(self._state)
	end
	-- 炸矿
	if self._state.craftString == "c:374627" then
		zhakuang_end=true
		for i,v in ipairs(zhakuang_types) do
			local CanSalvage=self:GetElement("content.selection.content.itemSelector")
				:CanSalvage(v)
			if CanSalvage  then
				self:GetElement("content.selection.content.itemSelector")
	...
	--搓宝石尘
	if self._state.craftString == "c:395696" then
		for i,v in ipairs(baoshichen_types) do
			local CanSalvage=self:GetElement("content.selection.content.itemSelector")
				:CanSalvage(v)
			if CanSalvage  then
				self:GetElement("content.selection.content.itemSelector")
...
function CraftDetails:SetScript(script, handler)
	...
	elseif script == "OnZhaKuangEnd" then
		self._onZhaKuangEnd = handler
	elseif script == "OnCuoXianEnd" then
		self._onCuoXianEnd = handler
...
function CraftDetails.__private:_HandleItemSelectionChanged(itemSelector)
	...
	--print("_HandleItemSelectionChanged")
	--print(self:GetElement("content.selection.content.itemSelector")
	--		  :GetSelection())
	self._state.neededTools = private.CraftStringToNeededTools(self._state.craftString)
	self._state.numCraftable = self:_GetSalvageCraftNum() or self._craftableQuantityFunc(self._state.recipeString)
end
...
function CraftDetails.__private:_HandleDialogRecipeStringChanged(_, recipeString)
	if self._state.craftString == "c:376509:q5" then
		self._state.recipeString = string.gsub(recipeString,"192095","192096")
		self:__closure("_HandleMaxBtnOnClick")()
	else
...
function CraftDetails.__private:_UpdateFenJie(disable)
	self:GetElement("content.buttons.FenJie"):SetDisabled(disable)
	if disable then
		self:GetElement("content.buttons.FenJie"):SetText("不可分解")
	else
		self:GetElement("content.buttons.FenJie"):SetText("分解")
	end
	self:GetElement("header"):Draw()
end
...
function private.StateToDestroyDisabled(state)
	return state.destroyFuture or not state.canDestroy
end
...
function CraftDetails.__private:DestroyButtonPreClick(button)
	assert(not self._state.destroyFuture)
	local selection=private.query:GetFirstResult()
	local future = OC.Destroying.StartMyDestroy(private.query)
	--if future == nil then
	--	return
	--end
	future:SetScript("OnDone", self:__closure("DestroyFutureOnDone"))
	self._state.destroyFuture = future
end
function private.FenJieCountUpdated(state)
	state.canDestroy =private.query:Count()>0
end
function CraftDetails.__private:DestroyFutureOnDone()
	if  self._state.destroyFuture  then
		self._state.destroyFuture:GetValue()
		self._state.destroyFuture = nil
	end
	private.FenJieCountUpdated(self._state)
end
