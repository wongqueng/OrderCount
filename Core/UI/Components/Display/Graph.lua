...
function Graph:__init()
	...
	frame.plot:OCSetScript("OnEnter", self:__closure("_HandlePlotEnter"))
	frame.plot:OCSetScript("OnLeave", self:__closure("_HandlePlotLeave"))
	frame.plot:OCSetScript("OnMouseDown", self:__closure("_HandlePlotMouseDown"))
	frame.plot:OCSetScript("OnMouseUp", self:__closure("_HandlePlotMouseUp"))
...
function Graph:Acquire()
	...
	frame:OCSubscribeBackdropColor("PRIMARY_BG")
	frame.plot.hoverText:OCSetFont("TABLE_TABLE1")
	frame.plot.hoverText:OCSubscribeTextColor("INDICATOR_ALT")

	-- Setup subscriptions on previously-created lines
	for _, line in ipairs(self._xAxisLines) do
		line:OCSubscribeVertexColor("ACTIVE_BG")
		line.text:OCSubscribeTextColor("TEXT")
	end
...
function Graph:_CreateAxisLine(axis)
	...
	line:SetTexture("Interface\\AddOns\\OrderCount\\Media\\line.tga")
	line:OCSubscribeVertexColor("ACTIVE_BG")
	...
	if axis == "X" then
		...
		line.text:OCSetFont("BODY_BODY3_MEDIUM")
	...
	elseif axis == "Y" then
		...
		line.text:OCSetFont("TABLE_TABLE1")
...
function Graph:_CreateSlice()
	...
	slice:SetTexture("Interface\\AddOns\\OrderCount\\Media\\line.tga")
	slice.fillTop = UIElements.CreateTexture(self, plot, "ARTWORK", -1)
	slice.fillTop:SetTexture("Interface\\AddOns\\OrderCount\\Media\\triangle")
...
function Graph.__private:_HandlePlotEnter()
	...
	self:Draw()
	local plot = self:_GetBaseFrame().plot
	plot:OCSetScript("OnUpdate", self:__closure("_HandlePlotUpdate"))
end
