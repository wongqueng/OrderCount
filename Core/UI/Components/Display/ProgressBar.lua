...
function ProgressBar:Acquire()
	...
	frame.barMask:OCSubscribeColorTexture("PRIMARY_BG")
	...
	self._state:PublisherForKeyChange("color")
		:CallMethod(frame.progressIcon, "OCSubscribeVertexColor")

	-- Set the icon visibility
	self._state:PublisherForKeyChange("iconVisible")
		:CallMethod(frame.progressIcon, "OCSetShown")

	-- Set the animation playing state
	self._state:PublisherForKeyChange("iconVisible")
		:CallMethod(frame.progressIcon.ag, "OCSetPlaying")
end

function ProgressBar:Release()
	local frame = self:_GetBaseFrame()
	frame.barMask:OCCancelAll()
	frame.progressIcon:OCCancelAll()
