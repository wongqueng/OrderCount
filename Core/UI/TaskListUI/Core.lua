...
local TaskListUI = OC.UI:NewPackage("TaskListUI")
...
function TaskListUI.OnInitialize()
	...
	OC.TaskList.SetUpdateCallback(private.OnTaskListUpdate)
...
function TaskListUI.Toggle()
	if private.frame then
		...
		if OC.TaskList.GetNumTasks() == 0 then
...
function TaskListUI.UpdateFrame()
	...
	private.frame:SetRoundedBackgroundColor((mouseOver or OC.db.global.appearanceOptions.taskListBackgroundLock) and "FRAME_BG%50" or nil)
	private.frame:SetBorderColor((mouseOver or OC.db.global.appearanceOptions.taskListBackgroundLock) and "ACTIVE_BG%50" or nil, 2)
	private.frame:Draw()
end
...
function private.CreateTaskListElements(frame)
	...
	for _, task in OC.TaskList.Iterator() do
...
function private.CloseBtnOnClick(button)
	Log.PrintUser(L["Hiding the OC Task List UI. Type '/oc tasklist' to reopen it."])
...
function private.OnTaskListUpdate()
	if private.frame then
		local numTasks = OC.TaskList.GetNumTasks()
		if numTasks == 0 then
			private.didAutoShow = false
			TaskListUI.Toggle()
			return
		end
		private.frame:SetTitle(L["OC TASK LIST"].." ("..numTasks..")")
	...
	elseif not private.didAutoShow and OC.TaskList.GetNumTasks() > 0 then
		TaskListUI.Toggle()
		private.didAutoShow = true
	end
end
