...
local Contacts = OC.UI.Util:NewPackage("Contacts")
...
function private.GenerateListElements(category)
	...
	if category == L["Recent"] then
		for k in pairs(OC.db.global.mailingOptions.recentlyMailedList) do
			local character = Ambiguate(k, "none")
			tinsert(private.listElements, character)
		end
	elseif category == L["Alts"] then
		for factionrealm, isConnected in OC.db:GetConnectedRealmIterator("realm") do
