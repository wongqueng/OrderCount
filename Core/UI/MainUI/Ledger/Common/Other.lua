...
local Other = OC.MainUI.Ledger.Common:NewPackage("Other")
...
function Other.OnInitialize()
	...
	OC.MainUI.Ledger.Expenses.RegisterPage(OTHER, private.DrawOtherExpensesPage)
	OC.MainUI.Ledger.Revenue.RegisterPage(OTHER, private.DrawOtherRevenuePage)
end
...
function private.DrawOtherPage(recordType)
	...
	for _, character in OC.Accounting.Money.CharacterIterator(recordType) do
	...
	if not private.query then
		private.query = OC.Accounting.Money.CreateQuery()
			:OrderBy("time", false)
	end
