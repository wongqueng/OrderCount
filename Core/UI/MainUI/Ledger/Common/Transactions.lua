...
local Transactions = OC.MainUI.Ledger.Common:NewPackage("Transactions")
...
function Transactions.OnInitialize()
	...
	OC.MainUI.Ledger.Expenses.RegisterPage(L["Purchases"], private.DrawPurchasesPage)
	OC.MainUI.Ledger.Revenue.RegisterPage(L["Sales"], private.DrawSalesPage)
end
...
function private.DrawTransactionPage()
	private.query = private.query or OC.Accounting.Transactions.CreateQuery()
	...
	private.query:Reset()
		...
		:LeftJoin(OC.Groups.GetItemDBForJoin(), "itemString")
...
function private.TableSelectionChanged(scrollingTable, row)
	OC.MainUI.Ledger.ShowItemDetail(scrollingTable:GetParentElement():GetParentElement(), row:GetField("itemString"), private.type)
end
