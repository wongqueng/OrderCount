...
local Inventory = OC.MainUI.Ledger:NewPackage("Inventory")
...
function Inventory.OnInitialize()
	...
	OC.MainUI.Ledger.RegisterPage(L["Inventory"], private.DrawInventoryPage)
end
...
function Inventory.OnEnable()
	...
	private.query = private.db:NewQuery()
		...
		:VirtualField("groupPath", "string", OC.Groups.GetPathByItem, "levelItemString")
		:OrderBy("name", true)
		:NotEqual("name", "")
end
