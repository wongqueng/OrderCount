...
local CustomSources = OC.MainUI.Settings:NewPackage("CustomSources")
...
function CustomSources.OnInitialize()
	OC.MainUI.Settings.RegisterSettingPage(L["Custom Sources"], "middle", private.GetCustomSourcesSettingsFrame)
end
...
function private.GetCustomSourcesSettingsFrame()
	...
	return UIElements.New("ScrollFrame", "content")
		:SetPadding(8, 8, 8, 0)
		:AddChild(OC.MainUI.Settings.CreateExpandableSection("Custom Price", "general", L["Custom Sources"], format(L["Custom sources allow you to create more advanced prices for use throughout the addon. You'll be able to use these new variables in the same way you can use the built-in price sources such as %s and %s."], Theme.GetColor("INDICATOR"):ColorText("vendorsell"), Theme.GetColor("INDICATOR"):ColorText("vendorbuy")), 60)
...
function private.CreateCustomPriceRow(name)
	local priceString = OC.db.global.userData.customPriceSources[name]
...
function private.EditCustomPriceOnClick(editBtn)
	...
	editBtn:GetBaseElement():ShowDialogFrame(OC.UI.Views.CustomStringDialog.New(value, L["Custom Source"], private.ValueValidateFunc, nil, private.EditDialogOnHide, editBtn)
...
function private.AddNewButtonOnClick(button)
	...
	while not newName do
		for i = strbyte("a"), strbyte("z") do
			...
			if not OC.db.global.userData.customPriceSources[newName] then
				break
			end
			newName = nil
		end
