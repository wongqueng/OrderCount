...
local Appearance = OC.MainUI.Settings:NewPackage("Appearance")
local L = OC.Include("Locale").GetTable()
local Color = OC.Include("Util.Color")
...
local MAGIC_STR = "OC_THEME_EXPORT"
local VERSION = 1
...
function Appearance.OnInitialize()
	...
	OC.MainUI.Settings.RegisterSettingPage(L["Appearance"], "middle", private.GetSettingsFrame)
end
...
function private.GetSettingsFrame()
	...
	local frame = UIElements.New("ScrollFrame", "generalSettings")
		:SetPadding(8, 8, 8, 0)
		:AddChild(OC.MainUI.Settings.CreateExpandableSection("Appearance", "appearance", L["General Options"], L["Some general appearance options are below."])
			:AddChild(UIElements.New("Frame", "content")
				...
				:AddChild(UIElements.New("Checkbox", "minimapCheckbox")
					...
					:SetSettingInfo(OC.db.global.coreOptions.minimapIcon, "hide")
			...
			:AddChild(UIElements.New("Frame", "content")
				...
				:AddChild(UIElements.New("Checkbox", "taskListLockCheckbox")
					...
					:SetSettingInfo(OC.db.global.appearanceOptions, "taskListBackgroundLock")
			...
			:AddChild(UIElements.New("Frame", "content")
				...
				:AddChild(UIElements.New("Checkbox", "showTotalMoneyCheckbox")
					...
					:SetSettingInfo(OC.db.global.appearanceOptions, "showTotalMoney")
				)
				:AddChild(UIElements.New("Spacer", "spacer"))
			)
		)
...
function private.CreateThemes(frame)
	for _, key, name, isActive in OC.UI.Util.ColorSetIterator() do
...
function private.CreateCustomColors(frame)
	for i, key in ipairs(THEME_KEYS) do
		local colorThemeKey = OC.UI.Util.GetCustomColorThemeKey(key)
		frame:AddChild(UIElements.New("Frame", key)
			:SetLayout("VERTICAL")
			:AddChild(UIElements.New("Text", "label")
				:SetHeight(20)
...
function private.MinimapOnValueChanged(_, value)
	if value then
		LibDBIcon:Hide("OrderCount")
	else
		LibDBIcon:Show("OrderCount")
	end
end

function private.TaskListLockOnValueChanged()
	if OC.UI.TaskListUI.IsVisible() then
		OC.UI.TaskListUI.UpdateFrame()
	end
end
...
function private.ThemeButtonOnClick(buttonToggle)
	...
	OC.UI.Util.SetActiveColorSet(selectedKey)
	local customKey = OC.UI.Util.GetCustomColorSetKey()
	...
	for _, key, _, isActive in OC.UI.Util.ColorSetIterator() do
...
function private.ImportInputOnValueChanged(input)
	...
	for key, hexColor in pairs(themeSet) do
		...
		OC.UI.Util.SetCustomColor(key, r, g, b)
		private.frame:GetElement("custom."..key..".frame.text")
...
function private.HexValueChanged(text, newValue)
	...
	if strmatch(strtrim(newValue), "^#([0-9a-f][0-9a-f])([0-9a-f][0-9a-f])([0-9a-f][0-9a-f])$") then
		...
		OC.UI.Util.SetCustomColor(THEME_KEYS[text:GetContext()], r, g, b)
		local themeContainer = private.frame:GetElement("theme")
		themeContainer:ReleaseAllChildren()
		themeContainer:AddChildrenWithFunction(private.CreateThemes)
		themeContainer:Draw()
...
function private.ColorOnMouseUp(frame)
	...
	local r, g, b = Theme.GetColor(OC.UI.Util.GetCustomColorThemeKey(private.currentThemeKey)):GetFractionalRGBA()
...
function private.SetCurrentThemeColor(r, g, b)
	...
	g = Math.Round(g * 255)
	b = Math.Round(b * 255)
	OC.UI.Util.SetCustomColor(private.currentThemeKey, r, g, b)
