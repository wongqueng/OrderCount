...
local Tooltip = OC.MainUI.Settings:NewPackage("Tooltip")
...
function Tooltip.OnInitialize()
	OC.MainUI.Settings.RegisterSettingPage(L["Tooltip Settings"], "top", private.GetTooltipSettingsFrame)
end
...
function private.GetTooltipSettingsFrame()
	...
	for _, moduleName in OC.Operations.ModuleIterator() do
		tinsert(private.operationModules, moduleName)
		tinsert(private.operationModuleNames, OC.Operations.GetLocalizedName(moduleName))
	end
	...
	for _, key, _, label in CustomPrice.Iterator() do
		...
		if not INVALID_DESTROY_PRICE_SOURCES[key] then
			...
			if OC.db.global.coreOptions.destroyValueSource == key then
	...
	if not foundCurrentSetting then
		...
		OC.db.global.coreOptions.destroyValueSource = strlower(OC.db:GetDefaultReadOnly("global", "coreOptions", "destroyValueSource"))
	end
	return UIElements.New("ScrollFrame", "tooltipSettings")
		:SetPadding(8, 8, 8, 0)
		:AddChild(OC.MainUI.Settings.CreateExpandableSection("Tooltip", "general", L["General Options"], L["Some general options for the OC tooltip information are below."])
			:AddChild(UIElements.New("Text", "label")
				:SetHeight(20)
				:SetMargin(0, 0, 0, 4)
				:SetFont("BODY_BODY2_MEDIUM")
				:SetText(L["Enable OC tooltips"])
			)
			:AddChild(UIElements.New("ToggleYesNo", "enableToggle")
				:SetHeight(24)
				:SetMargin(0, 0, 0, 12)
				:SetSettingInfo(OC.db.global.tooltipOptions, "enabled")
			)
			:AddChild(UIElements.New("Frame", "content")
				...
				:AddChild(UIElements.New("Checkbox", "embedCheckbox")
					:SetWidth("AUTO")
					:SetFont("BODY_BODY2_MEDIUM")
					:SetText(L["Embed OC tooltip"])
					:SetSettingInfo(OC.db.global.tooltipOptions, "embeddedTooltip")
				)
			...
			:AddChild(UIElements.New("Frame", "dropdownRow1")
				...
				:AddChild(UIElements.New("SelectionDropdown", "priceFormatDropdown")
					...
					:SetSettingInfo(OC.db.global.tooltipOptions, "tooltipPriceFormat")
				...
				:AddChild(UIElements.New("SelectionDropdown", "modifierDropdown")
					...
					:SetSettingInfo(OC.db.global.tooltipOptions, "tooltipShowModifier")
				)
				:AddChild(UIElements.New("SelectionDropdown", "dropdown")
					:SetItems(private.destroySources, private.destroySourceKeys)
					:SetSettingInfo(OC.db.global.coreOptions, "destroyValueSource")
					:SetScript("OnSelectionChanged", private.OnSettingChange)
				)
			)
		)
		:AddChild(OC.MainUI.Settings.CreateExpandableSection("Tooltip", "options", L["Tooltip Options"], L["Use the settings below to control which lines are shown in tooltips."])
			:AddChild(UIElements.New("Frame", "content")
				:SetLayout("HORIZONTAL")
...
function private.GetSettingsHeight()
	...
	height = height + 32 + #VALUES_SETTINGS_INFO * 32 + Table.Count(OC.db.global.userData.customPriceSources) * 32
...
function private.AddTooltipSettings(frame)
	...
	for name in pairs(OC.db.global.userData.customPriceSources) do
	...
	for _, name in ipairs(customPriceSources) do
		frame:AddChild(UIElements.New("Checkbox", "checkbox_"..name)
			...
			:SetSettingInfo(OC.db.global.tooltipOptions.customPriceTooltips, name)
...
function private.GetSettingTableFromInfo(info)
	local settingTbl = OC.db.global.tooltipOptions
...
function private.AddExampleTooltip(frame)
	for i, left, right, lineColor in OC.Tooltip.SettingsLineIterator() do
