...
local Settings = OC.MainUI:NewPackage("Settings")
...
function Settings.OnInitialize()
	OC.MainUI.RegisterTopLevelPage(L["Settings"], private.GetSettingsFrame)
end
	...
	return UIElements.New("Frame", id)
		...
		:AddChild(UIElements.New("Frame", "content")
			...
			:AddChild(UIElements.New("Input", "input")
				...
				:SetSettingInfo(OC.db[scope][namespace], key)
			...
			:AddChild(UIElements.New("ActionButton", "resetButton")
				:SetWidth(108)
				:SetText(L["Reset"])
				:SetDisabled(forceDisable or OC.db[scope][namespace][key] == OC.db:GetDefault(scope, namespace, key))
		...
		:AddChild(UIElements.New("Frame", "content")
			...
			:AddChild(UIElements.New("Frame", "reset")
				...
				:AddChild(UIElements.New("ActionButton", "button")
					:SetWidth(108)
					:SetText(L["Reset"])
					:SetDisabled(OC.db[scope][namespace][key] == OC.db:GetDefault(scope, namespace, key))
...
function Settings.PromptToReload()
	StaticPopupDialogs["OCReloadPrompt"] = StaticPopupDialogs["OCReloadPrompt"] or {
	...
	Wow.ShowStaticPopupDialog("OCReloadPrompt")
end
...
function private.InputOnValueChanged(input)
	...
	button:SetDisabled(OC.db[scope][namespace][key] == OC.db:GetDefault(scope, namespace, key))
		:Draw()
end

function private.ResetBtnOnClick(button)
	local scope, namespace, key = strsplit(".", button:GetContext())
	local defaultValue = OC.db:GetDefault(scope, namespace, key)
	OC.db:Set(scope, nil, namespace, key, defaultValue)
