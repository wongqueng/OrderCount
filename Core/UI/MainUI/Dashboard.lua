...
local Dashboard = OC.MainUI:NewPackage("Dashboard")
...
function Dashboard.OnInitialize()
	...
	OC.MainUI.RegisterTopLevelPage(L["Dashboard"], private.GetDashboardFrame)
end
...
function private.GetDashboardFrame()
	...
	for characterGuild in OC.Accounting.GoldTracker.CharacterGuildIterator() do
	...
	local frame = UIElements.New("DividedContainer", "dashboard")
		...
		:SetRightChild(UIElements.New("Frame", "content")
			...
			:AddChild(UIElements.New("Graph", "goldGraph")
				...
				:SetXRange(OC.Accounting.GoldTracker.GetGraphTimeRange(private.settings.dashboardUnselectedCharacters))
	...
	local newsEntries = OC.AppHelper.GetNews()
	if newsEntries then
...
function private.GetGraphYValue(xValue)
	return OC.Accounting.GoldTracker.GetGoldAtTime(xValue, private.settings.dashboardUnselectedCharacters)
end
...
function private.UpdateGraph(contentFrame)
	-- update the graph
	local minTime, maxTime = OC.Accounting.GoldTracker.GetGraphTimeRange(private.settings.dashboardUnselectedCharacters)
...
function private.PopulateDetails(contentFrame)
	...
	local query = OC.Accounting.GetSummaryQuery(timeFilterStart, timeFilterEnd, unselectedCharacters)
