...
local Auctioning = OC.MainUI.Operations:NewPackage("Auctioning")
...
function Auctioning.OnInitialize()
	...
	POST_CAP_VALIDATE_CONTEXT.minValue, POST_CAP_VALIDATE_CONTEXT.maxValue = OC.Operations.Auctioning.GetMinMaxValues("postCap")
	if Environment.HasFeature(Environment.FEATURES.AH_STACKS) then
		STACK_SIZE_VALIDATE_CONTEXT.minValue, STACK_SIZE_VALIDATE_CONTEXT.maxValue = OC.Operations.Auctioning.GetMinMaxValues("stackSize")
	end
	KEEP_QUANTITY_VALIDATE_CONTEXT.minValue, KEEP_QUANTITY_VALIDATE_CONTEXT.maxValue = OC.Operations.Auctioning.GetMinMaxValues("keepQuantity")
	MAX_EXPIRES_VALIDATE_CONTEXT.minValue, MAX_EXPIRES_VALIDATE_CONTEXT.maxValue = OC.Operations.Auctioning.GetMinMaxValues("maxExpires")
	OC.MainUI.Operations.RegisterModule("Auctioning", private.GetAuctioningOperationSettings)
end
...
function private.GetDetailsSettings()
	...
	local operation = OC.Operations.GetSettings("Auctioning", private.currentOperationName)
	return UIElements.New("ScrollFrame", "settings")
		:SetPadding(8, 8, 8, 0)
		:SetBackgroundColor("PRIMARY_BG")
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Auctioning", "generalOptions", L["General Options"], L["Adjust some general settings."])
			:AddChild(UIElements.New("Frame", "content")
				:SetLayout("VERTICAL")
				:AddChildrenWithFunction(private.AddMaxStackSizeSetting)
				:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("ignoreLowDuration", L["Ignore auctions by duration"])
					:SetLayout("VERTICAL")
					:SetHeight(48)
					:SetMargin(0, 0, 0, 16)
					:AddChild(UIElements.New("SelectionDropdown", "dropdown")
						:SetHeight(24)
						:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "ignoreLowDuration"))
		...
		:AddChild(OC.MainUI.Operations.GetOperationManagementElements("Auctioning", private.currentOperationName))
end
...
function private.GetPostingSettings()
	...
	return UIElements.New("ScrollFrame", "settings")
		:SetPadding(8, 8, 8, 0)
		:SetBackgroundColor("PRIMARY_BG")
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Auctioning", "postingSettings", L["Posting Options"], L["Adjust the settings below to set how groups attached to this operation will be auctioned."])
			:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("duration", L["Auction duration"])
				:SetLayout("VERTICAL")
				:SetHeight(48)
				:SetMargin(0, 0, 0, 12)
				:AddChild(UIElements.New("SelectionDropdown", "dropdown")
					:SetHeight(24)
					:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "duration"))
					:SetItems(OC.CONST.AUCTION_DURATIONS)
					:SetSelectedItemByKey(operation.duration)
					:SetScript("OnSelectionChanged", private.SetAuctioningDuration)
				)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("postCap", L["Post cap"], POST_CAP_VALIDATE_CONTEXT)
				:SetMargin(0, 0, 0, 12)
			)
			:AddChildrenWithFunction(private.AddStackSizeSettings)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("keepQuantity", L["Amount kept in bags"], KEEP_QUANTITY_VALIDATE_CONTEXT)
				:SetMargin(0, 0, 0, 12)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("maxExpires", L["Don't post after this many expires"], MAX_EXPIRES_VALIDATE_CONTEXT)
				:SetMargin(0, 0, 0, 4)
			)
		)
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Auctioning", "priceSettings", L["Posting Price"], L["Adjust the settings below to set how groups attached to this operation will be priced."])
			:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("bidPercent", L["Set bid as percentage of buyout"])
				:SetLayout("VERTICAL")
				:SetHeight(48)
				:SetMargin(0, 0, 0, 12)
				:AddChild(UIElements.New("Frame", "content")
					...
					:AddChild(UIElements.New("Input", "input")
						...
						:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "bidPercent"))
					...
					:AddChild(UIElements.New("Text", "label")
						...
						:SetTextColor(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "bidPercent") and "TEXT_DISABLED" or "TEXT")
					)
				)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("undercut", L["Undercut amount"], UNDERCUT_VALIDATE_CONTEXT)
				:SetMargin(0, 0, 0, 12)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("minPrice", L["Minimum price"], PRICE_VALIDATE_CONTEXT))
			:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("priceReset", L["When below minimum:"])
				:SetMargin(0, 0, 12, 12)
				:AddChild(UIElements.New("SelectionDropdown", "dropdown")
					:SetWidth(240)
					:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "priceReset"))
					:SetItems(BELOW_MIN_ITEMS, BELOW_MIN_KEYS)
					:SetSettingInfo(operation, "priceReset")
				)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("maxPrice", L["Maximum price"], PRICE_VALIDATE_CONTEXT))
			:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("aboveMax", L["When above maximum:"])
				:SetMargin(0, 0, 12, 12)
				:AddChild(UIElements.New("SelectionDropdown", "dropdown")
					:SetWidth(240)
					:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "aboveMax"))
					:SetItems(ABOVE_MAX_ITEMS, ABOVE_MAX_KEYS)
					:SetSettingInfo(operation, "aboveMax")
				)
			)
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("normalPrice", L["Normal price"], PRICE_VALIDATE_CONTEXT))
		)
end
...
function private.AddStackSizeSettings(frame)
	...
	frame:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("stackSize", L["Stack size"], STACK_SIZE_VALIDATE_CONTEXT)
		:SetMargin(0, 0, 0, 12)
	)
...
function private.GetCancelingSettings()
	...
	return UIElements.New("ScrollFrame", "settings")
		:SetPadding(8, 8, 8, 0)
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Auctioning", "priceSettings", L["Canceling Options"], L["Adjust the settings below to set how groups attached to this operation will be cancelled."])
			...
			:AddChild(OC.MainUI.Operations.CreateLinkedPriceInput("cancelRepostThreshold", L["Repost threshold"]))
		)
end
...
function private.AddBlacklistPlayers(frame)
	...
	frame:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine("blacklist", L["Blacklisted players"])
		:SetLayout("VERTICAL")
		:SetHeight(48)
		:AddChild(UIElements.New("Input", "input")
			:SetHeight(24)
			...
			:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, "blacklist"))
...
function private.CreateToggleLine(key, label)
	local operation = OC.Operations.GetSettings("Auctioning", private.currentOperationName)
	return OC.MainUI.Operations.CreateLinkedSettingLine(key, label)
		:SetLayout("VERTICAL")
		:SetHeight(48)
		:SetMargin(0, 0, 0, 12)
		:AddChild(UIElements.New("ToggleYesNo", "toggle")
			:SetHeight(18)
			:SetDisabled(OC.Operations.HasRelationship("Auctioning", private.currentOperationName, key))
