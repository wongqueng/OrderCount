...
local Operations = OC.MainUI:NewPackage("Operations")
...
function Operations.OnInitialize()
	...
	OC.MainUI.RegisterTopLevelPage(L["Operations"], private.GetOperationsFrame)
end
...
function Operations.GetOperationManagementElements(moduleName, operationName)
	local operation = OC.Operations.GetSettings(private.currentModule, private.currentOperationName)
	wipe(private.factionrealmList)
	wipe(private.playerList)
	for _, factionrealm in OC.db:ScopeKeyIterator("factionrealm") do
		tinsert(private.factionrealmList, factionrealm)
		for _, character in OC.db:FactionrealmCharacterIterator(factionrealm) do
		...
		:AddChild(Operations.CreateExpandableSection(moduleName, "managementOptions", L["Management Options"], L["Below you can ignore this operation on certain characters or realms."])
			:AddChild(Operations.CreateSettingLine("ignoreFactionRealms", L["Ignore operation on faction-realms"])
				...
				:AddChild(UIElements.New("MultiselectionDropdown", "dropdown")
					...
					:SetScript("OnSelectionChanged", OC.Groups.RebuildDB)
				)
			)
...
function Operations.CreateLinkedSettingLine(settingKey, labelText, disabled, alternateName)
	local relationshipSet = OC.Operations.HasRelationship(private.currentModule, private.currentOperationName, settingKey)
...
function Operations.CreateLinkedPriceInput(settingKey, label, validateContext, defaultValue, isDisabled)
	...
	isDisabled = isDisabled or OC.Operations.HasRelationship(private.currentModule, private.currentOperationName, settingKey)
	local operation = OC.Operations.GetSettings(private.currentModule, private.currentOperationName)
...
function private.GetSummaryContent()
	local query = OC.Operations.CreateQuery()
...
function private.AddOperationGroups(frame)
	for _, groupPath in OC.Operations.GroupIterator(private.currentModule, private.currentOperationName, true) do
		frame:AddChild(private.CreateGroupOperationLine(groupPath))
	end
end

function private.CreateGroupOperationLine(groupPath)
	local groupName = groupPath == OC.CONST.ROOT_GROUP_PATH and L["Base Group"] or GroupPath.GetName(groupPath)
	local level = select('#', strsplit(OC.CONST.GROUP_SEP, groupPath))
...
function private.OperationTreeOnOperationAdded(operationTree, moduleName, operationName, copyOperationName)
	...
	if copyOperationName then
		OC.Operations.Copy(moduleName, operationName, copyOperationName)
	end
end
...
function private.OperationTreeOnOperationDeleted(self, moduleName, operationName)
	OC.Operations.Delete(moduleName, operationName)
...
function private.OperationTreeOnOperationSelected(self, moduleName, operationName)
	...
	if moduleName and operationName then
		OC.Operations.Update(moduleName, operationName)
	...
	elseif moduleName then
		...
		for _ in OC.Operations.OperationIterator(moduleName) do
...
function private.CreateNewOperationOnClick(button)
	...
	local num = 1
	while OC.Operations.Exists(private.currentModule, operationName.." "..num) do
		num = num + 1
	end
	operationName = operationName .. " " .. num
	OC.Operations.Create(private.currentModule, operationName)
...
function private.OperationNameChanged(text, newValue)
	...
	elseif newValue == "" then
		...
		text:Draw()
	elseif OC.Operations.Exists(private.currentModule, newValue) then
		Log.PrintUser(L["Group already exists."])
		text:Draw()
	else
		OC.Operations.Rename(private.currentModule, private.currentOperationName, newValue)
...
function private.ConfirmResetOnClick(button)
	OC.Operations.Reset(private.currentModule, private.currentOperationName)
	...
	contentFrame:ReleaseAllChildren()
	OC.Operations.Update(private.currentModule, private.currentOperationName)
...
function private.GroupSelectionChanged(groupSelector)
	for groupPath in groupSelector:SelectedGroupIterator() do
		if not OC.Operations.GroupHasOperation(private.currentModule, groupPath, private.currentOperationName) then
			local parentElement = groupSelector:GetParentElement():GetParentElement()
			if groupPath ~= OC.CONST.ROOT_GROUP_PATH then
				OC.Groups.SetOperationOverride(groupPath, private.currentModule, true, true)
			end
			local numOperations = 0
			local lastOperationName = nil
			for _, groupOperationName in OC.Groups.OperationIterator(groupPath, private.currentModule) do
				lastOperationName = groupOperationName
				numOperations = numOperations + 1
			end
			if numOperations == OC.Operations.GetMaxNumber(private.currentModule) then
				-- replace the last operation since we're already at the max number of operations
				OC.Groups.RemoveOperation(groupPath, private.currentModule, numOperations, true)
			...
			OC.Groups.AppendOperation(groupPath, private.currentModule, private.currentOperationName, true)
			Log.PrintfUser(L["Added %s to %s."], Log.ColorUserAccentText(private.currentOperationName), Log.ColorUserAccentText(groupPath == OC.CONST.ROOT_GROUP_PATH and L["Base Group"] or GroupPath.Format(groupPath)))
...
function private.ViewGroupOnClick(button)
	...
	OC.MainUI.Groups.ShowGroupSettings(baseFrame, button:GetContext())
end

function private.RemoveOperationGroupOnClick(self)
	local groupPath = self:GetContext()
	OC.Groups.RemoveOperationByName(groupPath, private.currentModule, private.currentOperationName)
...
function private.LinkBtnOnClick(button)
	...
	for _, operationName in OC.Operations.OperationIterator(private.currentModule) do
		if operationName ~= private.currentOperationName and not OC.Operations.IsCircularRelationship(private.currentModule, private.currentOperationName, settingKey) then
	...
	button:GetBaseElement():ShowDialogFrame(UIElements.New("PopupFrame", "linkDialog")
		...
		:AddChild(UIElements.New("SelectionList", "list")
			:SetContext(settingKey)
			:SetMargin(2, 2, 0, 3)
			:SetEntries(private.linkMenuEntries, OC.Operations.GetRelationship(private.currentModule, private.currentOperationName, settingKey))
...
function private.ListOnEntrySelected(list, operationName)
	...
	local previousValue = OC.Operations.GetRelationship(private.currentModule, private.currentOperationName, settingKey)
	if operationName == previousValue then
		OC.Operations.SetRelationship(private.currentModule, private.currentOperationName, settingKey, nil)
	else
		OC.Operations.SetRelationship(private.currentModule, private.currentOperationName, settingKey, operationName)
	end
...
function private.DeleteSelectedOperations(scrollingTable)
	...
	OC.Operations.DeleteList(private.currentModule, toDelete)
...
function private.NumGroupsVirtualField(row)
	local num = 0
	for _ in OC.Operations.GroupIterator(row:GetField("moduleName"), row:GetField("operationName")) do
		num = num + 1
	end
	return num
end
...
function private.NumItemsVirtualField(row)
	...
	local num = 0
	for _, groupPath in OC.Operations.GroupIterator(row:GetField("moduleName"), row:GetField("operationName")) do
		if groupPath == OC.CONST.ROOT_GROUP_PATH then
			includesBaseGroup = true
		else
			num = num + OC.Groups.GetNumItems(groupPath)
		end
	end
...
function private.CustomStringPopoutOnClick(button)
	...
	button:GetBaseElement():ShowDialogFrame(OC.UI.Views.CustomStringDialog.New(input:GetValue(), title, validateFunc, validateContext.badSources, validateContext.isNumber and private.CustomStringNumberDialogCallback or private.CustomStringDialogCallback, button))
end
