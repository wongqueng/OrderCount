...
local Warehousing = OC.MainUI.Operations:NewPackage("Warehousing")
...
function Warehousing.OnInitialize()
	OC.MainUI.Operations.RegisterModule("Warehousing", private.GetWarehousingOperationSettings)
end
...
function private.GetWarehousingOperationSettings(operationName)
	...
	return UIElements.New("ScrollFrame", "settings")
		:SetPadding(8, 8, 8, 0)
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Warehousing", "moveSettings", L["Move Quantity Options"], L["Set how items are moved out of the bank."])
		...
		:AddChild(OC.MainUI.Operations.CreateExpandableSection("Warehousing", "restockSettings", L["Restock Options"], L["Set how items are restocked from the bank."])
		...
		:AddChild(OC.MainUI.Operations.GetOperationManagementElements("Warehousing", private.currentOperationName))
end

function private.CreateEnabledSettingLine(key, enableText, text, minValue, maxValue, margin)
	local operation = OC.Operations.GetSettings("Warehousing", private.currentOperationName)
	local hasRelationship = OC.Operations.HasRelationship("Warehousing", private.currentOperationName, key)
	return UIElements.New("Frame", "content")
			:SetLayout("VERTICAL")
			:AddChild(OC.MainUI.Operations.CreateLinkedSettingLine(key, text)
				:SetLayout("VERTICAL")
				:SetHeight(48)
				:SetMargin(0, 0, 0, 12)
			...
			:AddChild(OC.MainUI.Operations.CreateSettingLine("content", text, hasRelationship or operation[key] == 0)
				:SetLayout("VERTICAL")
				:SetHeight(48)
