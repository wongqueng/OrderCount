...
local Groups = OC.MainUI:NewPackage("Groups")
local L = OC.Include("Locale").GetTable()
local Analytics = OC.Include("Util.Analytics")
...
local SmartMap = OC.Include("Util.SmartMap")
...
local ItemFilter = OC.Include("Service.ItemFilter")
...
local private = {
	settings = nil,
	currentGroupPath = OC.CONST.ROOT_GROUP_PATH,
	moveGroupPath = nil,
...
function Groups.OnInitialize()
	...
	OC.MainUI.RegisterTopLevelPage(L["Groups"], private.GetGroupsFrame)
...
function private.GetGroupsFrame()
	...
	private.currentGroupPath = OC.CONST.ROOT_GROUP_PATH
	...
	local frame = UIElements.New("DividedContainer", "groups")
		...
		:SetLeftChild(UIElements.New("Frame", "groupSelection")
			...
			:AddChild(UIElements.New("ManagementGroupTree", "groupTree")
				...
				:SetQuery(OC.Groups.CreateQuery())
	...
	return UIElements.New("Frame", "content")
		...
		:AddChild(UIElements.New("Frame", "bottom")
			...
			:AddChild(UIElements.New("GroupSelector", "group")
				...
				:AddCreateNew()
				:SetSelection(OC.Groups.Exists(private.moveGroupPath) and private.moveGroupPath)
...
function private.GetGroupsPage(_, button)
	...
	elseif button == L["Items"] then
		...
		assert(private.currentGroupPath ~= OC.CONST.ROOT_GROUP_PATH)
		local hasParent = GroupPath.GetParent(private.currentGroupPath) ~= OC.CONST.ROOT_GROUP_PATH
...
function private.GetModuleOperationFrame(moduleName)
	local override = OC.Groups.HasOperationOverride(private.currentGroupPath, moduleName) or private.currentGroupPath == OC.CONST.ROOT_GROUP_PATH
	local numGroupOperations = 0
	for _ in OC.Groups.OperationIterator(private.currentGroupPath, moduleName) do
	...
	return UIElements.New("CollapsibleContainer", "operationInfo"..moduleName)
		...
		:AddChildIf(private.currentGroupPath ~= OC.CONST.ROOT_GROUP_PATH, UIElements.New("Checkbox", "overrideCheckbox")
			:SetHeight(20)
			:SetMargin(0, 0, 6, 0)
			:SetFont("BODY_BODY2")
		...
		:AddChildIf(override and numGroupOperations < OC.Operations.GetMaxNumber(moduleName), UIElements.New("Frame", "addMore")
			:SetLayout("HORIZONTAL")
			:SetHeight(24)
			:SetMargin(0, 0, 8, 0)
...
function private.AddOperationRows(container)
	...
	local override = OC.Groups.HasOperationOverride(private.currentGroupPath, moduleName) or private.currentGroupPath == OC.CONST.ROOT_GROUP_PATH
	for i, operationName in OC.Groups.OperationIterator(private.currentGroupPath, moduleName) do
...
function private.DragButtonOnMouseUp(button)
	...
	for k, v in container:LayoutChildrenIterator() do
		if k ~= index and v:IsMouseOver() then
			OC.Groups.SwapOperation(private.currentGroupPath, moduleName, index, k)
		end
	end
	for i, operationName in OC.Groups.OperationIterator(private.currentGroupPath, moduleName) do
...
function private.GroupSearchOnValueChanged(input)
	...
	if strmatch(strlower(private.currentGroupPath), searchStr) then
		...
		if private.currentGroupPath == OC.CONST.ROOT_GROUP_PATH then
			titleFrame:GetElement("text")
				:SetTextColor("TEXT")
				:SetText(L["Base Group"])
		else
			titleFrame:GetElement("text")
				:SetTextColor(Theme.GetGroupColorKey(select('#', strsplit(OC.CONST.GROUP_SEP, private.currentGroupPath))))
		...
		local titleFrame = groupsContentFrame:GetElement("header.title")
		local buttonsFrame = groupsContentFrame:GetElement("buttons")
		input:GetElement("__parent.__parent.groupTree"):SetSelectedGroup(OC.CONST.ROOT_GROUP_PATH)
		titleFrame:GetElement("text")
...
function private.ImportInputOnValueChanged(input)
	...
	if not OC.Groups.ImportExport.ProcessImport(input:GetValue()) then
	...
	local importGroupName = OC.Groups.ImportExport.GetPendingImportGroupName()
	for groupPath in OC.Groups.ImportExport.PendingImportGroupIterator() do
		groupPath = groupPath == OC.CONST.ROOT_GROUP_PATH and importGroupName or GroupPath.Join(importGroupName, groupPath)
		local orderStr = gsub(groupPath, OC.CONST.GROUP_SEP, "\001")
...
function private.UpdateImportConfirmationDialog(dialogFrame)
	local numItems, numGroups, numExistingItems, numOperations, numExistingOperations, numExistingCustomSources = OC.Groups.ImportExport.GetImportTotals()
...
function private.ImportGroupTreeOnGroupSelectionChanged(groupTree)
	...
	for relativeGroupPath in OC.Groups.ImportExport.PendingImportGroupIterator() do
		local groupPath = relativeGroupPath == OC.CONST.ROOT_GROUP_PATH and importGroupName or GroupPath.Join(importGroupName, relativeGroupPath)
		local isSelected = groupTree:IsGroupSelected(groupPath)
		OC.Groups.ImportExport.SetGroupFiltered(relativeGroupPath, not isSelected)
		if isSelected then
...
function private.ImportBtnOnClick(button)
	...
	OC.Groups.ImportExport.CommitImport(moveExistingItems, includeOperations, replaceOperations)
	button:GetBaseElement():HideDialog()
end

function private.ImportOnHide()
	OC.Groups.ImportExport.ClearImportContext()
end
...
function private.GroupTreeOnGroupSelected(groupTree, path)
	...
	if path == OC.CONST.ROOT_GROUP_PATH then
		...
		local groupColorKey = Theme.GetGroupColorKey(select('#', strsplit(OC.CONST.GROUP_SEP, path)))
		titleFrame:GetElement("icon")
...
function private.GroupTreeOnDragRecieved(groupTree, context, destPath)
	...
	for _, itemString in ipairs(context) do
		if OC.Groups.GetPathByItem(itemString) ~= destPath then
			tinsert(items, itemString)
		end
	end
	OC.Groups.SetItemsGroup(items, destPath ~= OC.CONST.ROOT_GROUP_PATH and destPath or nil)
...
function private.GroupNameChanged(text, newValue)
	...
	local newPath = parent and parent ~= OC.CONST.ROOT_GROUP_PATH and (parent..OC.CONST.GROUP_SEP..newValue) or newValue
	if newPath == private.currentGroupPath then
		-- didn't change
		text:Draw()
	elseif strfind(newValue, OC.CONST.GROUP_SEP) or newValue == "" then
		Log.PrintUser(L["Invalid group name."])
		text:Draw()
	elseif OC.Groups.Exists(newPath) then
		Log.PrintUser(L["Group already exists."])
		text:Draw()
	else
		OC.Groups.Move(private.currentGroupPath, newPath)
...
function private.ExportBtnOnClick(button)
	...
	for _, groupPath in OC.Groups.GroupIterator() do
		...
		if relGroupPath then
			...
			local orderStr = gsub(relGroupPath, OC.CONST.GROUP_SEP, "\001")
			orderStr = strlower(orderStr)
	...
	local str, numItems, numSubGroups, numOperations, numCustomSources = OC.Groups.ImportExport.GenerateExport(private.currentGroupPath, private.exportSubGroups, false, false)
	local coloredGroupName = Theme.GetGroupColor(select('#', strsplit(OC.CONST.GROUP_SEP, private.currentGroupPath))):ColorText(GroupPath.GetName(private.currentGroupPath))
	button:GetBaseElement():ShowDialogFrame(UIElements.New("Frame", "frame")
		:SetLayout("VERTICAL")
		:SetSize(658, 408)
		:SetPadding(12)
...
function private.ExportOptionOnValueChanged(element)
	...
	local str, numItems, numSubGroups, numOperations, numCustomSources = OC.Groups.ImportExport.GenerateExport(private.currentGroupPath, private.exportSubGroups, excludeOperations, excludeCustomSources)
...
function private.ExportInputOnEnterPressed(input)
	input:SetValue(OC.Groups.ImportExport.GenerateExport(private.currentGroupPath, private.exportSubGroups, false, false))
		:Draw()
end
...
function private.UpdateBaseItemInfoQuery()
	...
	private.baseItemInfoQuery = ItemInfo.MatchItemFilterQuery(private.itemFilter, private.baseItemInfoQuery)
		...
		:LeftJoin(OC.Groups.GetItemDBForJoin(), "ungroupedItemString", "itemString")
		:IsNil("groupPath")
		:OrderBy("name", true)
...
function private.BaseMoveItemOnClick(button)
	...
	OC.Groups.SetItemsGroup(addedItems, private.moveGroupPath)
...
function private.AddItemsOnClick(button)
	...
	local addedItems = TempTable.Acquire()
	itemList:GetSelectedItems(addedItems)
	OC.Groups.SetItemsGroup(addedItems, private.currentGroupPath)
...
function private.GroupedItemsOnSelectionChanged(itemList)
	...
	parentGroup = parentGroup ~= OC.CONST.ROOT_GROUP_PATH and parentGroup or nil
	if parentGroup then
...
function private.RebuildModuleOperations(moduleOperationFrame)
	...
	moduleOperationFrame:AddChildIf(override and numGroupOperations < OC.Operations.GetMaxNumber(moduleName), UIElements.New("Frame", "addMore")
		:SetLayout("HORIZONTAL")
		:SetHeight(24)
		:SetMargin(0, 0, 8, 0)
		:AddChild(UIElements.New("Spacer", "spacer"))
...
function private.RemoveItemsOnClick(button)
	...
	OC.Groups.SetItemsGroup(removedItems, targetGroup)
...
function private.OverrideToggleOnValueChanged(checkbox, value)
	...
	local moduleName = moduleOperationFrame:GetContext()
	OC.Groups.SetOperationOverride(private.currentGroupPath, moduleName, value)
...
function private.ConfigOperationOnClick(button)
	...
	OC.MainUI.Operations.ShowOperationSettings(baseFrame, moduleName, operationName)
end

function private.AddOperationButtonOnClick(button)
	local moduleName = button:GetContext()
	private.operationQuery = private.operationQuery or OC.Operations.CreateQuery()
...
function private.AddOperationCreateBtnOnClick(button)
	...
	local extra = ""
	local num = 0
	while OC.Operations.Exists(moduleName, operationName..extra) do
		num = num + 1
		extra = " "..num
	end
	operationName = operationName..extra
	OC.Operations.Create(moduleName, operationName)
	OC.Groups.AppendOperation(private.currentGroupPath, moduleName, operationName)
	Analytics.Action("ADDED_GROUP_OPERATION", private.currentGroupPath, moduleName, operationName)
	OC.MainUI.Operations.ShowOperationSettings(button:GetBaseElement(), moduleName, operationName)
end
...
function private.AddOperationBtnOnClick(button)
	...
	OC.Groups.AppendOperation(private.currentGroupPath, scrollingTable:GetContext(), operationName)
	Analytics.Action("ADDED_GROUP_OPERATION", private.currentGroupPath, moduleName, operationName)
...
function private.RemoveOperationOnClick(button)
	...
	for index, name in OC.Groups.OperationIterator(private.currentGroupPath, moduleName) do
		if index == operationIndex then
			operationName = name
		end
	end
	assert(operationName)
	OC.Groups.RemoveOperation(private.currentGroupPath, moduleName, operationIndex)
...
function private.CreateParentGroupItemQuery()
	...
	return OC.Groups.CreateItemsQuery(parentGroupPath, false)
...
function private.CreateGroupedItemQuery()
	return OC.Groups.CreateItemsQuery(private.currentGroupPath, true)
		:Select("itemString", "texture", "coloredItemName")
		:VirtualField("name", "string", ItemInfo.GetName, "itemString", "?")
		:VirtualField("texture", "number", ItemInfo.GetTexture, "itemString", ItemInfo.GetTexture(ItemString.GetUnknown()))
		:VirtualField("coloredItemName", "string", UIUtils.GetDisplayItemName, "itemString", Theme.GetColor("FEEDBACK_RED"):ColorText("?"))
