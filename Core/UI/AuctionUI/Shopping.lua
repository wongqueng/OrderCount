...
local Shopping = OC.UI.AuctionUI:NewPackage("Shopping")
local Environment = OC.Include("Environment")
local ItemClass = OC.Include("Data.ItemClass")
...
function Shopping.OnInitialize()
	...
	private.postTimeStr = OC.CONST.AUCTION_DURATIONS[2]
	private.selectionChangedTimer = Delay.CreateTimer("SHOPPING_SELECTION_CHANGED", private.AuctionsOnSelectionChangedDelayed)
	OC.UI.AuctionUI.RegisterTopLevelPage(L["Browse"], private.GetShoppingFrame, private.OnItemLinked)
	private.FSMCreate()
end
...
function Shopping.IsVisible()
	return OC.UI.AuctionUI.IsPageOpen(L["Browse"])
end
	...
	local frame = UIElements.New("DividedContainer", "selection")
		...
		:SetLeftChild(UIElements.New("Frame", "groupSelection")
			...
			:AddChild(UIElements.New("ApplicationGroupTree", "groupTree")
				...
				:SetQuery(OC.Groups.CreateQuery(), "Shopping")
	...
	return UIElements.New("Frame", "search")
		...
		:AddChild(UIElements.New("Frame", "buttonsLine2")
			...
			:AddChild(UIElements.New("ActionButton", "dealsBtn")
				...
				:SetDisabled(not OC.Shopping.GreatDealsSearch.GetFilter())
...
function private.GetSearchesElement(self, button)
	if button == L["Recent Searches"] then
		return UIElements.New("SearchList", "list")
			:SetQuery(OC.Shopping.SavedSearches.CreateRecentSearchesQuery())
	...
	elseif button == L["Favorite Searches"] then
		return UIElements.New("SearchList", "list")
			:SetQuery(OC.Shopping.SavedSearches.CreateFavoriteSearchesQuery())
			:SetScript("OnFavoriteChanged", private.SearchListOnFavoriteChanged)
...
function private.GetScanFrame()
	...
	local frame = UIElements.New("Frame", "scan")
		...
		:AddChild(UIElements.New("Frame", "bottom")
			...
			:AddChild(UIElements.NewNamed("ActionButton", "buyoutBtn", "OCShoppingBuyoutBtn")
				:SetSize(107, 24)
				:SetText(BUYOUT)
				:SetDisabled(true)
...
function private.ScanButtonOnClick(button)
	...
	local searchContext = OC.Shopping.GroupSearch.GetSearchContext(private.selectedGroups)
...
function private.SearchListOnFavoriteChanged(_, dbRow, isFavorite)
	OC.Shopping.SavedSearches.SetSearchIsFavorite(dbRow, isFavorite)
end
...
function private.RenameInputOnEnterPressed(input)
	...
	OC.Shopping.SavedSearches.RenameSearch(dbRow, name)
end
...
function private.SearchListOnDelete(_, dbRow)
	OC.Shopping.SavedSearches.DeleteSearch(dbRow)
end
...
function private.StartSearchHelper(viewContainer, searchContext, filter, errMsg)
	if not OC.UI.AuctionUI.StartingScan(L["Browse"]) then
		return
	end
...
function private.StartFilterSearchHelper(viewContainer, filter, isGreatDeals, postContext)
	...
	if isGreatDeals then
		searchContext = OC.Shopping.FilterSearch.GetGreatDealsSearchContext(filter)
	else
		searchContext, errMsg = OC.Shopping.FilterSearch.GetSearchContext(filter, postContext)
	end
...
function private.StartGatheringSearchHelper(viewContainer, items, stateCallback, buyCallback, mode)
	...
	local searchContext = OC.Shopping.FilterSearch.GetGatheringSearchContext(filter, mode)
	assert(searchContext)
...
function private.DealsButtonOnClick(button)
	...
	private.StartFilterSearchHelper(viewContainer, OC.Shopping.GreatDealsSearch.GetFilter(), true)
end

function private.VendorButtonOnClick(button)
	local viewContainer = button:GetParentElement():GetParentElement():GetParentElement():GetParentElement():GetParentElement()
	local searchContext = OC.Shopping.VendorSearch.GetSearchContext()
	assert(searchContext)
	private.StartSearchHelper(viewContainer, searchContext)
end
...
function private.DisenchantButtonOnClick(button)
	local viewContainer = button:GetParentElement():GetParentElement():GetParentElement():GetParentElement():GetParentElement()
	local searchContext = OC.Shopping.DisenchantSearch.GetSearchContext()
	assert(searchContext)
	private.StartSearchHelper(viewContainer, searchContext)
end
...
function private.PerItemOnClick(button)
	...
	if postContext.quantity == stackSizeEdit then
		local newBid = OC.UI.AuctionUI.ParseBid(bidInput:GetValue())
		...
		local newBuyout = OC.UI.AuctionUI.ParseBuyout(buyoutInput:GetValue(), isCommodity)
...
function private.PerStackOnClick(button)
	...
	local isCommodity = ItemInfo.IsCommodity(private.itemString)
	if postContext.quantity == stackSizeEdit then
		newBid = OC.UI.AuctionUI.ParseBid(bidInput:GetValue())
		...
		newBuyout = OC.UI.AuctionUI.ParseBuyout(buyoutInput:GetValue(), isCommodity)
...
function private.BidBuyoutValidateFunc(input, value)
	...
	if input:GetContext() == "bid" then
		...
		value, errMsg = OC.UI.AuctionUI.ParseBuyout(value, ItemInfo.IsCommodity(private.itemString))
	end
	if not value then
...
function private.BidBuyoutInputOnValueChanged(input)
	...
	local buyout = OC.UI.AuctionUI.ParseBuyout(buyoutInput:GetValue(), ItemInfo.IsCommodity(private.itemString))
...
function private.UpdateDepositCostAndPostButton(frame)
	...
	local postTime = Table.GetDistinctKey(OC.CONST.AUCTION_DURATIONS, private.postTimeStr)
...
function private.PostButtonOnClick(button)
	...
	local stackSize = tonumber(frame:GetElement("quantity.input"):GetValue())
	local bid = OC.UI.AuctionUI.ParseBid(frame:GetElement("bid.input"):GetValue())
	local buyout = OC.UI.AuctionUI.ParseBuyout(frame:GetElement("buyout.input"):GetValue(), ItemInfo.IsCommodity(private.itemString))
	...
	local postTime = Table.GetDistinctKey(OC.CONST.AUCTION_DURATIONS, frame:GetElement("duration.toggle"):GetValue())
	...
	private.fsm = FSM.New("SHOPPING")
		:AddState(FSM.NewState("ST_INIT")
			:SetOnEnter(function(context, searchContext)
				...
				OC.UI.AuctionUI.EndedScan(L["Browse"])
			end)
		...
		:AddState(FSM.NewState("ST_RESULTS")
			:SetOnEnter(function(context, didBuy)
				...
				if selection and selection:IsSubRow() then
					if OC.UI.AuctionUI.StartingScan(L["Browse"]) then
		...
		:AddState(FSM.NewState("ST_FINDING_AUCTION")
			:SetOnEnter(function(context)
				...
				context.cancelShown = false
				UpdateScanFrame(context)
				OC.Shopping.SearchCommon.StartFindAuction(context.auctionScan, context.findAuction, private.FSMFindAuctionCallback, false)
			end)
			:SetOnExit(function(context)
				context.auctionScan:Cancel()
				OC.Shopping.SearchCommon.StopFindAuction(true)
			end)
			:AddTransition("ST_FINDING_AUCTION")
			:AddTransition("ST_RESULTS")
			:AddTransition("ST_UPDATING_SCAN_PROGRESS")
		...
		:AddState(FSM.NewState("ST_BUY_CONFIRMATION")
			:SetOnEnter(function(context)
				...
				if OC.UI.AuctionUI.BuyUtil.ShowConfirmation(context.scanFrame, selection, true, context.numConfirmed + 1, context.defaultBuyQuantity, context.maxQuantity, private.FSMConfirmationCallback, context.auctionScan, index, false, context.searchContext) then
					return "ST_BUYING"
				else
		...
		:AddState(FSM.NewState("ST_BID_CONFIRMATION")
			:SetOnEnter(function(context)
				local selection = context.scanFrame:GetElement("auctions"):GetSelection()
				local index = not Environment.IsRetail() and context.findResult[#context.findResult] or nil
				if OC.UI.AuctionUI.BuyUtil.ShowConfirmation(context.scanFrame, selection, false, context.numConfirmed + 1, context.defaultBuyQuantity, context.maxQuantity, private.FSMConfirmationCallback, context.auctionScan, index, false, context.searchContext) then
					return "ST_BUYING"
				else
