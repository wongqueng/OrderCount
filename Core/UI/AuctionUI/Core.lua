...
local AuctionUI = OC.UI:NewPackage("AuctionUI")
...
function private.AuctionFrameInit()
	...
	if not private.hasShown then
		...
		if Environment.IsRetail() then
			LibAHTab:CreateTab(AH_TAB_ID, CreateFrame("Frame"), Theme.GetColor("INDICATOR_ALT"):ColorText("OC"))
			ScriptWrapper.Set(LibAHTab:GetButton(AH_TAB_ID), "OnClick", private.OCTabOnClick)
			...
			tab:Hide()
			tab:SetID(tabId)
			tab:SetText(Theme.GetColor("INDICATOR_ALT"):ColorText("OC"))
			...
			ScriptWrapper.Set(tab, "OnClick", private.OCTabOnClick)
		end
	end
...
function private.CreateMainFrame()
	...
	local whatsNewDialog = OC.UI.WhatsNew.GetDialog()
