...
local MyAuctions = OC.UI.AuctionUI:NewPackage("MyAuctions")
...
function MyAuctions.OnInitialize()
	...
	private.FSMCreate()
	OC.UI.AuctionUI.RegisterTopLevelPage(L["My Auctions"], private.GetMyAuctionsFrame, private.OnItemLinked)
end
...
function private.GetMyAuctionsFrame()
	...
	private.query = private.query or OC.MyAuctions.CreateQuery()
	...
	frame:AddChild(UIElements.New("HorizontalLine", "line"))
		:AddChild(UIElements.New("Frame", "bottom")
			...
			:AddChild(UIElements.New("Frame", "row2")
				...
				:AddChild(UIElements.NewNamed("ActionButton", "cancelBtn", "OCCancelAuctionBtn")
					:SetSize(110, 24)
					:SetMargin(0, 8, 0, 0)
					:SetText(CANCEL)
					:SetDisabled(true)
...
function private.FSMCreate()
	...
	local function UpdateFrame(context)
		...
		local numPending = OC.MyAuctions.GetNumPending()
	...
	private.fsm = FSM.New("MY_AUCTIONS")
		...
		:AddState(FSM.NewState("ST_CANCELING")
			:SetOnEnter(function(context)
				...
				if OC.MyAuctions.CanCancel(auctionId) then
					OC.MyAuctions.CancelAuction(auctionId)
				end
				return "ST_SHOWN"
			end)
			:AddTransition("ST_HIDDEN")
