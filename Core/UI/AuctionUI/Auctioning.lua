...
local Auctioning = OC.UI.AuctionUI:NewPackage("Auctioning")
...
local Sound = OC.Include("Util.Sound")
...
local AuctionScan = OC.Include("Service.AuctionScan")
...
function Auctioning.OnInitialize()
	...
	OC.UI.AuctionUI.RegisterTopLevelPage(L["Auctioning"], private.GetAuctioningFrame, private.OnItemLinked)
	private.FSMCreate()
end
	...
	local frame = UIElements.New("DividedContainer", "selection")
		...
		:SetLeftChild(UIElements.New("Frame", "groupSelection")
			...
			:AddChild(UIElements.New("ApplicationGroupTree", "groupTree")
				...
				:SetQuery(OC.Groups.CreateQuery(), "Auctioning")
			...
			:AddChild(UIElements.New("Frame", "bottom")
				...
				:AddChild(UIElements.New("HorizontalLine", "line"))
				:AddChild(UIElements.NewNamed("ActionButton", "postScanBtn","MyPostScanBtn")
					:SetHeight(24)
					:SetMargin(8)
					:SetText(L["Run Post Scan"])
					:SetScript("OnClick", private.RunPostButtonOnclick)
				)
				:AddChild(UIElements.NewNamed("ActionButton", "cancelScanBtn","MyCancelScanBtn")
					:SetHeight(24)
					:SetMargin(8, 8, 0, 8)
				...
				:AddChild(UIElements.New("SelectionScrollingTable", "bagScrollingTable")
					...
					:GetScrollingTableInfo()
						...
						:NewColumn("operation")
							...
							:Commit()
						:Commit()
					:SetQuery(OC.Auctioning.PostScan.CreateBagsQuery())
...
function private.GetScansElement(_, button)
	if button == L["Recent Scans"] then
		return UIElements.New("SearchList", "list")
			:SetQuery(OC.Auctioning.SavedSearches.CreateRecentSearchesQuery())
	...
	elseif button == L["Favorite Scans"] then
		return UIElements.New("SearchList", "list")
			:SetQuery(OC.Auctioning.SavedSearches.CreateFavoriteSearchesQuery())
			:SetScript("OnFavoriteChanged", private.SearchListOnFavoriteChanged)
	...
	return UIElements.New("Frame", "scan")
		...
		:AddChild(UIElements.New("Frame", "header")
			...
			:AddChild(UIElements.New("Frame", "back")
				...
				:AddChild(UIElements.NewNamed("Button", "backBtn","MyAutionBack")
					:SetMargin(0, 0, 2, 0)
					:SetSize(28, 28)
		...
		:AddChild(UIElements.New("Frame", "bottom")
			...
			:AddChild(UIElements.NewNamed("ActionButton", "processBtn", "OCAuctioningBtn")
				:SetSize(160, 24)
				:SetMargin(0, 8, 0, 0)
				:SetText(L["Post"])
				:SetDisabled(true)
...
function private.ScanNavCallback(_, path)
	if path == L["Auctioning Log"] then
		...
		private.logQuery = private.logQuery or OC.Auctioning.Log.CreateQuery()
			...
			:AddChild(UIElements.New("QueryScrollingTable", "log")
				...
				:GetScrollingTableInfo()
					...
					:NewColumn("info")
						...
						:SetFont("TABLE_TABLE1")
						:SetJustifyH("LEFT")
						:SetTextInfo(nil, OC.Auctioning.Log.GetInfoStr)
...
function private.OnItemLinked(_, itemLink)
	if private.selectionFrame then
		if not OC.UI.AuctionUI.StartingScan(L["Auctioning"]) then
			return false
		end
		wipe(private.scanContext)
		private.scanContext.isItems = true
		tinsert(private.scanContext, OC.Groups.TranslateItemString(ItemString.Get(itemLink)))
...
function private.SearchListOnFavoriteChanged(_, dbRow, isFavorite)
	OC.Auctioning.SavedSearches.SetSearchIsFavorite(dbRow, isFavorite)
end
...
function private.RenameInputOnEnterPressed(input)
	...
	OC.Auctioning.SavedSearches.RenameSearch(dbRow, name)
end
...
function private.SearchListOnDelete(_, dbRow)
	OC.Auctioning.SavedSearches.DeleteSearch(dbRow)
end
...
function private.SearchListOnRowClick(searchList, dbRow)
	...
	OC.Auctioning.SavedSearches.FiltersToTable(dbRow, private.scanContext)
...
function private.FSMCreate()
	...
	function fsmPrivate.UpdateDepositCost(context)
		...
		local currentRow = OC.Auctioning.PostScan.GetCurrentRow()
	...
	function fsmPrivate.UpdateScanFrame(context)
		...
		if context.scanType == "POST" then
			currentRow = OC.Auctioning.PostScan.GetCurrentRow()
			numProcessed, numConfirmed, _, totalNum = OC.Auctioning.PostScan.GetStatus()
			header:GetElement("content.item.cost")
				:Show()
				:Draw()
		elseif context.scanType == "CANCEL" then
			currentRow = OC.Auctioning.CancelScan.GetCurrentRow()
			numProcessed, numConfirmed, _, totalNum = OC.Auctioning.CancelScan.GetStatus()
			header:GetElement("content.item.cost")
				:Hide()
				:Draw()
		else
		...
		if currentRow then
			...
			local duration = nil
			if context.scanType == "POST" then
				duration = OC.CONST.AUCTION_DURATIONS[currentRow:GetField("postTime")]
			elseif context.scanType == "CANCEL" then
				if Environment.HasFeature(Environment.FEATURES.C_AUCTION_HOUSE) then
		...
		if not isPausePending and (scanDone or isPaused) then
			...
			if bottom:GetElement("processBtn"):GetDisabled() then
				if context.scanType == "POST" then
					bottom:GetElement("processBtn"):SetText("不能发布")
				elseif context.scanType == "CANCEL" then
					bottom:GetElement("processBtn"):SetText("不能取消")
				end
			end
		...
		context.scanFrame:GetBaseElement():ShowDialogFrame(UIElements.New("Frame", "frame")
			...
			:AddChild(UIElements.New("Frame", "duration")
				...
				:AddChild(UIElements.New("Toggle", "toggle")
					:SetMargin(0, 48, 0, 0)
					:AddOption(OC.CONST.AUCTION_DURATIONS[1])
					:AddOption(OC.CONST.AUCTION_DURATIONS[2])
					:AddOption(OC.CONST.AUCTION_DURATIONS[3])
					:SetOption(OC.CONST.AUCTION_DURATIONS[currentRow:GetField("postTime")])
				)
			)
	...
	function fsmPrivate.EditPostingDetailsSaveOnClick(button)
		...
		local bid = OC.UI.AuctionUI.ParseBid(button:GetElement("__parent.bid.input"):GetValue())
		local buyout = OC.UI.AuctionUI.ParseBuyout(button:GetElement("__parent.buyout.input"):GetValue(), isCommodity)
		if buyout > 0 then
			bid = min(bid, buyout)
		end
		if Environment.IsRetail() and isCommodity and bid ~= buyout then
			...
			local duration = Table.KeyByValue(OC.CONST.AUCTION_DURATIONS, button:GetElement("__parent.duration.toggle"):GetValue())
			if duration ~= currentRow:GetField("postTime") then
				OC.Auctioning.PostScan.ChangePostDetail("postTime", duration)
			end
			...
			if buyout ~= currentRow:GetField(private.perItem and "itemBuyout" or "buyout") then
				OC.Auctioning.PostScan.ChangePostDetail(private.perItem and "itemBuyout" or "buyout", buyout)
			end

			if not ItemInfo.IsCommodity(context.itemString) and bid ~= currentRow:GetField(private.perItem and "itemBid" or "bid") then
				OC.Auctioning.PostScan.ChangePostDetail(private.perItem and "itemBid" or "bid", bid)
			end
		end
	...
	private.fsm = FSM.New("AUCTIONING")
		:AddState(FSM.NewState("ST_INIT")
			:SetOnEnter(function(context, scanType, scanContext)
				...
				OC.Auctioning.Log.Truncate()
				OC.Auctioning.PostScan.Reset()
				OC.Auctioning.CancelScan.Reset()
				...
				OC.UI.AuctionUI.EndedScan(L["Auctioning"])
			end)
		...
		:AddState(FSM.NewState("ST_STARTING_SCAN")
			:SetOnEnter(function(context, scanType, scanContext)
				...
				if context.scanType == "POST" then
					context.scanThreadId = OC.Auctioning.PostScan.Prepare()
					private.logQuery:ResetOrderBy()
					private.logQuery:OrderBy("index", true)
				elseif context.scanType == "CANCEL" then
					context.scanThreadId = OC.Auctioning.CancelScan.Prepare()
					private.logQuery:ResetOrderBy()
		...
		:AddState(FSM.NewState("ST_HANDLING_CONFIRM")
			:SetOnEnter(function(context, success, canRetry)
				local isDone = false
				if context.scanType == "POST" then
					OC.Auctioning.PostScan.HandleConfirm(success, canRetry)
					local _, numConfirmed, numFailed, totalNum = OC.Auctioning.PostScan.GetStatus()
					if numConfirmed == totalNum then
						if numFailed > 0 then
							-- TODO: need to wait for the player's bags to settle
							Log.PrintfUser(L["Retrying %d auction(s) which failed."], numFailed)
							OC.Auctioning.PostScan.PrepareFailedPosts()
						else
							isDone = true
						end
					end
				elseif context.scanType == "CANCEL" then
					OC.Auctioning.CancelScan.HandleConfirm(success, canRetry)
					local _, numConfirmed, numFailed, totalNum = OC.Auctioning.CancelScan.GetStatus()
					if numConfirmed == totalNum then
						if numFailed > 0 then
							-- TODO: need to wait for the player's auctions to settle
							Log.PrintfUser(L["Retrying %d auction(s) which failed."], numFailed)
							OC.Auctioning.CancelScan.PrepareFailedCancels()
						else
							isDone = true
						end
					end
		...
		:AddState(FSM.NewState("ST_RESULTS")
			:SetOnEnter(function(context)
				...
				if  context.scanType == "POST" and totalNum==0 then
					return "ST_INIT"
				elseif context.scanType == "CANCEL"   then
					if not  context.scanFrame ==nil then
						local p_test=context.scanFrame:GetElement("bottom.progressBar"):GetText()
						if p_test==L["Done Canceling"] then
							return "ST_INIT"
						end
					end
				end
			...
			:AddEvent("EV_PROCESS_CLICKED", function(context)
				...
				if context.scanType == "POST" then
					result, noRetry = OC.Auctioning.PostScan.DoProcess()
				elseif context.scanType == "CANCEL" then
					result, noRetry = OC.Auctioning.CancelScan.DoProcess()
				else
					error("Invalid scan type: "..tostring(context.scanType))
				end
				if not result then
			...
			:AddEvent("EV_SKIP_CLICKED", function(context)
				local isDone = nil
				if context.scanType == "POST" then
					OC.Auctioning.PostScan.DoSkip()
					local _, numConfirmed, numFailed, totalNum = OC.Auctioning.PostScan.GetStatus()
					isDone = numConfirmed == totalNum and numFailed == 0
				elseif context.scanType == "CANCEL" then
					OC.Auctioning.CancelScan.DoSkip()
					local _, numConfirmed, numFailed, totalNum = OC.Auctioning.CancelScan.GetStatus()
					isDone = numConfirmed == totalNum and numFailed == 0
				else
					error("Invalid scan type: "..tostring(context.scanType))
...
function private.LogGetIndexIcon(row)
	if row:GetField("state") == "PENDING" then
		...
		return TextureAtlas.GetColoredKey("iconPack.12x12/Circle", OC.Auctioning.Log.GetColorFromReasonKey(row:GetField("reasonKey")))
	else
...
function private.PerItemOnClick(button)
	...
	local row = OC.Auctioning.PostScan.GetCurrentRow()
	...
	local bid = OC.UI.AuctionUI.ParseBid(bidInput:GetValue())
	local buyout = OC.UI.AuctionUI.ParseBuyout(buyoutInput:GetValue(), ItemInfo.IsCommodity(row:GetField("itemString")))
...
function private.UpdateSaveButtonState(frame)
	...
	local bid = OC.UI.AuctionUI.ParseBid(bidInput:GetValue())
	local buyout = OC.UI.AuctionUI.ParseBuyout(buyoutInput:GetValue(), isCommodity)
...
function private.BidBuyoutValidateFunc(input, value)
	...
	if context == "bid" then
		value, errMsg = OC.UI.AuctionUI.ParseBid(value)
	else
		local isCommodity = context == "itemBuyout"
		value, errMsg = OC.UI.AuctionUI.ParseBuyout(value, isCommodity)
	end
	if not value then
