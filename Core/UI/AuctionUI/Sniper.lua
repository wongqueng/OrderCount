...
local Sniper = OC.UI.AuctionUI:NewPackage("Sniper")
...
function Sniper.OnInitialize()
	...
	OC.UI.AuctionUI.RegisterTopLevelPage(L["Sniper"], private.GetSniperFrame, private.OnItemLinked)
	private.FSMCreate()
end
...
function private.GetScanFrame()
	...
	return UIElements.New("Frame", "scan")
		...
		:AddChild(UIElements.New("Frame", "bottom")
			...
			:AddChild(UIElements.NewNamed("ActionButton", "actionBtn", "OCSniperBtn")
				:SetSize(165, 24)
				:SetText(BID)
				:SetDisabled(true)
...
function private.OnItemLinked(name, itemLink)
	...
	OC.UI.AuctionUI.SetOpenPage(L["Browse"])
	OC.UI.AuctionUI.Shopping.StartItemSearch(itemLink)
	return true
end
...
function private.StartScanHelper(viewContainer, searchContext)
	if not OC.UI.AuctionUI.StartingScan(L["Sniper"]) then
		return
	end
...
function private.BuyoutScanButtonOnClick(button)
	...
	local searchContext = OC.Sniper.BuyoutSearch.GetSearchContext()
	private.StartScanHelper(viewContainer, searchContext)
end

function private.BidScanButtonOnClick(button)
	local viewContainer = button:GetParentElement():GetParentElement():GetParentElement()
	local searchContext = OC.Sniper.BidSearch.GetSearchContext()
	private.StartScanHelper(viewContainer, searchContext)
end
	...
	private.fsm = FSM.New("SNIPER")
		:AddState(FSM.NewState("ST_INIT")
			:SetOnEnter(function(context, searchContext)
				...
				OC.UI.AuctionUI.EndedScan(L["Sniper"])
			end)
		...
		:AddState(FSM.NewState("ST_FINDING_AUCTION")
			:SetOnEnter(function(context)
				...
				UpdateScanFrame(context)
				OC.Shopping.SearchCommon.StartFindAuction(context.auctionScan, context.findAuction, private.FSMFindAuctionCallback, true)
			end)
			:SetOnExit(function(context)
				OC.Shopping.SearchCommon.StopFindAuction()
			end)
			:AddTransition("ST_RESULTS")
			:AddTransition("ST_FINDING_AUCTION")
			:AddTransition("ST_AUCTION_FOUND")
		...
		:AddState(FSM.NewState("ST_BID_BUY_CONFIRMATION")
			:SetOnEnter(function(context)
				...
				if OC.UI.AuctionUI.BuyUtil.ShowConfirmation(context.scanFrame, selection, context.searchContext:IsBuyoutScan(), context.numConfirmed + 1, context.numFound, context.maxQuantity, private.FSMConfirmationCallback, context.auctionScan, index, true, context.searchContext) then
					return "ST_BIDDING_BUYING"
				else
