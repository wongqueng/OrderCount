...
function private.DoInitialize()
	...
	if private.totalInitializeTime == 0 then
		for _, path, moduleLoadTime, settingsLoadTime in OC.ModuleInfoIterator() do
...
function private.DoEnable()
	...
	if private.totalEnableTime == 0 then
		for _, path, _, _, gameDataLoadTime in OC.ModuleInfoIterator() do
...
function private.OnDisableHelper()
	...
	for _, addon in ipairs(private.disableQueue) do
		-- defer the main OC.OnDisable() call to the very end
		if addon.OnDisable and addon ~= OC then
...
do
	...
	local function EventHandler(self, event, arg)
		if event == "ADDON_LOADED" and arg == "OrderCount" then
			-- There seems to be a bug in 10.1.5 where another addon loading the Blizzard_Professions addon causes our
			-- frames to get ADDON_LOADED in a weird order - workaround by just making sure we load after LibOC
			if not OC.IsLibOCLoaded() then
			...
			elseif self == private.eventFrames[#private.eventFrames] then
				error("Ran out of event frames to initialize OC")
			end
		elseif event == "PLAYER_LOGIN" then
			...
			elseif self == private.eventFrames[#private.eventFrames] then
				error("Ran out of event frames to enable OC")
			end
		end
	end
...
local AddonPackage = LibOCClass.DefineClass("AddonPackage") ---@class AddonPackage
...
local Addon = LibOCClass.DefineClass("Addon", AddonPackage) ---@class Addon: AddonPackage
...
do
	LibOCClass.ConstructWithTable(OC, Addon, ADDON_NAME)
end
...
function OC.AddonTestLogout()
	private.OnDisableHelper()
	OC.DebugLogout()
	for _, path, _, _, _, moduleUnloadTime in OC.ModuleInfoIterator() do
