[![Coverage Status](https://coveralls.io/repos/github/OrderCount/LibOCClass/badge.svg?branch=main)](https://coveralls.io/github/OrderCount/LibOCClass?branch=main)

# LibOCClass

The LibOCClass library allows for writing objected-oriented code in lua! There are many OOP / class libraries out there for lua, but none of them had all the features which we needed for OrderCount, were easily imported into WoW, and were sufficiently performant.

## Features

### Class Definition

To define a new class, simply use the `LibOCClass.DefineClass()` method of the library:
```lua
local MyClass = LibOCClass.DefineClass("MyClass")
```
...
If a table containing existing attributes already exists, it can be converted into an instance of the class via the `LibOCClass.ConstructWithTable()` method.

```lua
local tbl = { existingValue = 2 }
local classInst = LibOCClass.ConstructWithTable(tbl, MyClass)
...
```lua
local MySubClass = LibOCClass.DefineClass("MySubClass", MyClass)
...
One of the most powerful features of LibOCClass is support for virtual class methods. What this means is that within a base class method, an instance of a class is still treated as its an instance of its actual class, not the base class. This is best demonstrated with an example:
...
```lua
local AbstractClass = LibOCClass.DefineClass("AbstractClass", nil, "ABSTRACT")

local ImplClass = LibOCClass.DefineClass("ImplClass", AbstractClass)
```

### Abstract Class Methods

Abstract classes may define abstract methods which subclasses are required to implement. This is done by defining an empty function against the `__abstract` table. Note that this function doesn't strictly need to be empty, but is never called (or even stored anywhere within LibOCClass). Abstract class methods are **always implicitly protected**, so must be overridden as such.
...
One gotcha of LibOCClass is that all methods and static fields of a class must be fully defined before that class is subclassed or instantiated. This means that changing the definition of a class at runtime is not supported, and may lead to undefined behavior. Along the same lines, once a class's methods are defined, they may not be changed later on.

### Highly-Performant Base Classes

Inheritance is one of the most powerful uses of OOP, and LibOCClass fully supports it. However, for cases where performance is of the utmost importance, LibOCClass is heavily optimized to reduce the overhead of a class which does not subclass anything to be as close to direct table access as possible (without metamethod calls).

## Example

A basic example of the library is below:
```lua
local LibOCClass = LibStub("LibOCClass")
local MyClass = LibOCClass.DefineClass("MyClass")
...
LibOCClass is licensed under the MIT license. See LICENSE.txt for more information. If you would like to contribute to LibOCClass, opening an issue or submitting a pull request against the [LibOCClass Bitbucket project](https://bitbucket.org/tradeskillmasteraddon/libocclass) is highly encouraged.
