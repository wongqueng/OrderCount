...
local Builder = OC.Init("Service.ItemTooltipClasses.Builder") ---@class Service.ItemTooltipClasses.Builder
...
local TooltipBuilder = LibOCClass.DefineClass("TooltipBuilder")
local private = {}
...
function TooltipBuilder.FormatMoney(self, value, color)
	...
	return Money.ToString(value, color:GetTextColorPrefix(), OC.db.global.tooltipOptions.tooltipPriceFormat == "icon" and "OPT_ICON" or nil, self._disabled and "OPT_DISABLE" or nil, "OPT_RETAIL_ROUND")
end
...
function TooltipBuilder._Prepare(self, itemString, quantity)
	...
	if InCombatLockdown() and itemString ~= ItemString.GetPlaceholder() then
		self:AddLine(L["Can't load OC tooltip while in combat"])
		return true
	else
		return false
	end
