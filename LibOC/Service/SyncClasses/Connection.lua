...
local Connection = OC.Init("Service.SyncClasses.Connection")
...
local Comm = OC.Include("Service.SyncClasses.Comm")
local private = {
	isActive = false,
...
function Connection.Establish(targetCharacter)
	if not private.hasFriendsInfo then
		Log.PrintUser(L["OC is not yet ready to establish a new sync connection. Please try again later."])
		return false
	end
