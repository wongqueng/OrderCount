...
local Comm = OC.Init("Service.SyncClasses.Comm") ---@class Service.SyncClasses.Comm
...
local Constants = OC.Include("Service.SyncClasses.Constants")
local private = {
	disconnectFunc = nil,
	handler = {},
	queuedPacket = {},
...
Comm:OnModuleLoad(function()
	...
	Comm:RegisterComm("OCSyncData", private.OnCommReceived)
end)
...
function Comm.SendData(dataType, targetCharacter, data)
	...
	-- Send the message
	Comm:SendCommMessage("OCSyncData", compressed, "WHISPER", targetCharacter, priority)
	return #compressed
end
