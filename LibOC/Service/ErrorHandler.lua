...
local ErrorHandler = OC.Init("Service.ErrorHandler")
...
local PRINT_PREFIX = "|cffff0000OC:|r "
...
function private.ErrorHandler(msg, thread)
	...
	msg = gsub(msg, "%.%.%.T?r?a?d?e?S?k?i?l?l?M?a?ster([_A-Za-z]*[\\/])", "OrderCount%1")
	msg = strsub(msg, strfind(msg, "OrderCount") or 1)
	msg = gsub(msg, "OrderCount([^%.])", "OC%1")
	...
	msg = newMsg
	local addonName = isSilent and "OrderCount" or nil
	for _, info in ipairs(stackInfo) do
		if not addonName then
			addonName = strmatch(info.file, "[A-Za-z]+%.lua") and private.IsOCAddon(info.file) or nil
		end
	end
	if not isManual and addonName ~= "OrderCount" then
		-- not a OC error
		private.ignoreErrors = false
		return false
	end
	...
	-- object pool info
	local status, objectPoolInfo = pcall(function() return OC.Include("Util.ObjectPool").GetDebugInfo() end)
	...
	status, threadInfoStr = pcall(function() return OC.Include("Service.Threading").GetDebugStr() end)
	...
	print(PRINT_PREFIX..L["Looks like OrderCount has encountered an error. Please help the author fix this error by following the instructions shown."])
...
function private.GetStackInfo(msg, thread)
	...
	for i, frame in ipairs(stackFrames) do
		...
		if prevFrame and strfind(frame.file, "LibOCClass%.lua") then
				...
				if frame.rawLocals then
					...
					if className then
						if Environment.IsDev() then
							prevFrame.localsStr = prevFrame.localsStr.."\n"..LibOCClass.GetDebugInfo(className..":"..objKey, 5, private.LocalTableLookupFunc)
						end
		...
		if not startIndex then
			if errLocation and strmatch(frame.file..":"..frame.line, "[A-Za-z]+%.lua:[0-9]+") == errLocation then
				startIndex = strfind(frame.file, "LibOCClass%.lua") and (i - 1) or i
	...
	if errLocation and strfind(errLocation, "LibOCClass%.lua:%d+") and stackFrames[1] and not strfind(stackFrames[1].file, "LibOCClass%.lua") then
		msg = gsub(msg, ".+LibOCClass%.lua:[0-9]+", stackFrames[1].file..":"..stackFrames[1].line)
	end
...
function private.GetStackFrame(level, thread)
	...
	stackLine = gsub(stackLine, "%.%.%.T?r?a?d?e?S?k?i?l?l?M?a?ster([_A-Za-z]*[\\/])", "OrderCount%1")
	...
	locationStr = strsub(locationStr, strfind(locationStr, "OrderCount") or 1)
	locationStr = gsub(locationStr, "OrderCount([^%.])", "OC%1")
...
function private.ParseLocals(locals, file)
	...
	for localLine in gmatch(locals, "[^\n]+") do
		...
		elseif strmatch(localLine, "LibOCClass%.lua:") then
		...
		if not shouldIgnoreLine then
			...
			localLine = gsub(localLine, "Interface[\\/][Aa]dd[Oo]ns[\\/]OrderCount", "OC")
			...
			if level > 0 then
				if isBlizzardFile then
					...
					shouldIgnoreLine = true
				elseif strmatch(localLine, "^ *[_]*[A-Z].+@OC") then
...
function private.LocalTableLookupFunc(tbl)
	local status, result = pcall(function() return OC.Include("Util.ReactiveClasses.State").GetDebugInfo(tbl) end)
	return status and result ~= "" and result or nil
end

function private.IsOCAddon(str)
	...
	elseif strfind(str, "Master_AppHelper[\\/]") then
		return "OrderCount_AppHelper"
	elseif strfind(str, "lMaster[\\/]") then
		return "OrderCount"
	elseif strfind(str, "ster[\\/]Core[\\/]UI[\\/]") then
		return "OrderCount"
	elseif strfind(str, "r[\\/]LibOC[\\/]") then
		return "OrderCount"
	elseif strfind(str, "^OC[\\/]") then
		return "OrderCount"
	end
	return nil
end
...
function private.AddonBlockedHandler(event, addonName, addonFunc)
	if not strmatch(addonName, "OrderCount") then
		return
	end
	-- just log it - it might not be OC that cause the taint
	...
	if Environment.IsDev() then
		...
		if not status and not private.hitInternalError then
			...
			print("Internal OC error: "..tostring(ret))
		end
	end
end
...
function private.CreateErrorFrame()
	...
	frame:SetScript("OnShow", function(self)
		...
		if self.showingError then
			...
			self.text:SetText("Looks like OrderCount has encountered an error.")
			self.switchBtn:SetText("Hide Error")
			self.editBox:SetText(self.errorStr)
		else
			self.text:SetText("Looks like OrderCount has encountered an error. Please provide the steps which lead to this error to help the OC team fix it, then click either button at the bottom of the window to automatically report this error.")
	...
	title:SetText("OC Error Window ("..Environment.GetVersion()..")")
...
do
	...
	local function ErrorHandlerFunc(errMsg)
		local ocErrMsg = strtrim(tostring(errMsg))
		if private.ignoreErrors then
			-- we're ignoring errors
			ocErrMsg = nil
		elseif strmatch(ocErrMsg, "auc%-stat%-wowuction") or strmatch(ocErrMsg, "TheUndermineJournal%.lua") or strmatch(ocErrMsg, "[\\/]SavedVariables[\\/]OrderCount") or strmatch(ocErrMsg, "AddOn OrderCount[_a-zA-Z]* attempted") or (strmatch(ocErrMsg, "ItemTooltipClasses[\\/]Wrapper%.lua:98") and strmatch(ocErrMsg, "SetQuest")) then
			-- explicitly ignore these errors
			ocErrMsg = nil
		elseif strmatch(ocErrMsg, "Blizzard_AuctionUI%.lua:751") then
			-- suppress this Blizzard error
			return true
		end
		if ocErrMsg then
			-- look at the stack trace to see if this is a OC error
			for i = 2, MAX_STACK_DEPTH do
				...
				if not strmatch(stackLine, "^%[C%]:") and not strmatch(stackLine, "%(tail call%):") and not strmatch(stackLine, "^%[string \"[^@]") and not strmatch(stackLine, "lMaster[\\/]External[\\/][A-Za-z0-9%-_%.]+[\\/]") and not strmatch(stackLine, "SharedXML") and not strmatch(stackLine, "CallbackHandler") and not strmatch(stackLine, "!BugGrabber") and not strmatch(stackLine, "ErrorHandler%.lua") then
					if not private.IsOCAddon(stackLine) then
						ocErrMsg = nil
					end
					break
				end
		...
		if ocErrMsg then
			local status, ret = pcall(private.ErrorHandler, ocErrMsg)
	...
	local wrapperFunc = assert(loadstring(WRAPPER_FUNC_STR, "=[oc error check]"))
