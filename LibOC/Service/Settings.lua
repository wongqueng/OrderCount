...
local Settings = OC.Init("Service.Settings") ---@class Service.Settings
...
local Reactive = OC.Include("Util.Reactive")
local private = {
	context = {},
	proxies = {},
	views = {},
...
-- [7] changed default value of 'ocItemTweetEnabled' to false
-- [8] added 'global.itemCacheVersion' key
-- [9] removed 'global.itemCacheVersion' key, added 'global.clientVersion' key
-- [10] first OC4 version - combined all module settings into a single DB
...
-- [54] removed global.coreOptions.{ocItemTweetEnabled,auctionSaleEnabled,auctionBuyEnabled}
...
local SETTINGS_INFO = {
	...
	global = {
		...
		mainUIContext = {
			...
			orderHistoryScrollingTable = { type = "table", default = { colWidth = { item = 160, player =100, client=150, orderType = 65,commission=100, quantity = 72, time = 150 }, colHidden = {} }, lastModifiedVersion = 59 },
	...
	realm = {
		internalData = {
			...
			csvOrderLog = { type = "string", default = "", lastModifiedVersion = 10 },
			...
			saveTimeOrderLog = { type = "string", default = "", lastModifiedVersion = 10 },
	...
	sync = {
		-- NOTE: whenever these are changed, the sync version needs to be increased in LibOC/Services/SyncClasses/Constants.lua
		internalData = {
...
Settings:OnSettingsLoad(function()
	local db, upgradeObj = private.Constructor("OrderDB", SETTINGS_INFO)
	private.db = db
	...
	if prevVersion < 19 then
		...
		for factionrealm, characters in pairs(oldInventoryData) do
			local syncMetadata = oldSyncMetadata[factionrealm] and oldSyncMetadata[factionrealm].OC_CHARACTERS
	...
	if prevVersion < 64 then
		...
		if hadValue then
			...
			StaticPopupDialogs["OC_ACCOUNTING_SMART_AVG_REMOVED"] = {
				text = L["The 'use smart average for purchase price' setting has been removed from OC and replaced with a new 'SmartAvgBuy' price source. Please update your custom prices appropriately."],
				button1 = OKAY,
				timeout = 0,
				whileDead = true,
			}
			Wow.ShowStaticPopupDialog("OC_ACCOUNTING_SMART_AVG_REMOVED")
		end
	end
...
function Settings.ShowSyncSVCopyError()
	...
	Log.PrintfUser(L["It appears that you've manually copied your saved variables between accounts which will cause OC's automatic sync'ing to not work. You'll need to undo this, and/or delete the OrderCount saved variables files on both accounts (with WoW closed) in order to fix this."])
end
...
-- ============================================================================
-- Proxy Class for Scopes (OC.db.XXXXX)
-- ============================================================================
...
-- ============================================================================
-- Proxy Class for Namespaces (OC.db.<scope>.XXXXX)
-- ============================================================================
...
function VIEW_METHODS:GetForScopeKey(key, ...)
	...
	local result= viewInfo.settingsDB:Get(viewInfo.scopeLookup[key], scopeKey, viewInfo.namespaceLookup[key], key)
	return result
end
