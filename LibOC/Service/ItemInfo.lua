...
ItemInfo:OnSettingsLoad(function()
	...
	local isValid = true
	if not OCItemInfoDB or OCItemInfoDB.version ~= DB_VERSION or OCItemInfoDB.locale ~= GetLocale() or OCItemInfoDB.build ~= build or OCItemInfoDB.revision ~= revision then
		isValid = false
	elseif #OCItemInfoDB.data % RECORD_DATA_LENGTH_CHARS ~= 0 then
		Analytics.Action("CORRUPTED_ITEM_INFO", "DATA", #OCItemInfoDB.data)
		isValid = false
	else
		names = private.LoadLongString(OCItemInfoDB.names)
		itemStrings = private.LoadLongString(OCItemInfoDB.itemStrings)
	...
	for i = 1, numItemsLoaded do
		...
		if ItemString.Get(itemString) == itemString then
			...
			local b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13, b14, b15, b16, b17, b18, b19, b20, b21, b22, b23, bExtra = strbyte(OCItemInfoDB.data, dataOffset, dataOffset + RECORD_DATA_LENGTH_CHARS - 1)
...
ItemInfo:OnModuleUnload(function()
	-- save the DB
	if not OCItemInfoDB or not private.hasChanged then
		-- bailing if OCItemInfoDB doesn't exist gives us an easy way to wipe the DB via "/run OCItemInfoDB = nil"
		return
	end
	local names = {}
	local itemStrings = {}
	...
	OCItemInfoDB.names = private.StoreLongString(names)
	OCItemInfoDB.itemStrings = private.StoreLongString(itemStrings)
	OCItemInfoDB.data = table.concat(dataParts)

	if #OCItemInfoDB.data % RECORD_DATA_LENGTH_CHARS ~= 0 then
		OCItemInfoDB = nil
		return
	end

	local build, revision = GetBuildInfo()
	OCItemInfoDB.version = DB_VERSION
	OCItemInfoDB.locale = GetLocale()
	OCItemInfoDB.build = build
	OCItemInfoDB.revision = revision
end)
...
function private.ShowRebuildMessage()
	...
	Log.PrintUser(L["OC is currently rebuilding its item cache which may cause FPS drops and result in OC not being fully functional until this process is complete. This is normal and typically takes a few minutes."])
