...
local Rectangle = OC.Init("UI.Rectangle") ---@class UI.Rectangle
local RectangleTexture = OC.Include("LibOCClass").DefineClass("RectangleTexture") ---@class RectangleTexture
...
function RectangleTexture:SubscribeColor(color)
	for _, key in ipairs(CENTER_TEXTURE_KEYS) do
		self._textures[key]:OCSubscribeColorTexture(color)
	end
	for key in pairs(CORNER_TEXTURE_INFO) do
		self._textures[key]:OCSubscribeVertexColor(color)
	end
end
