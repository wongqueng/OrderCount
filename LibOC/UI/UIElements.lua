...
local UIElements = OC.Init("UI.UIElements") ---@class UI.UIElements
...
---@param ... ClassProperties Any additional class modifiers to pass to LibOCClass.DefineClass()
---@return T
function UIElements.Define(elementType, parentElementType, ...)
	...
	assert(parentClass)
	local class = LibOCClass.DefineClass(elementType, parentClass, ...)
...
--- Gets a UI element by its frame (for OC's frame stack).
...
function UIElements.CreateFrame(element, frameType, frameName, parentFrame, template)
	...
	if not frameName then
		...
		frameName = private.GetDebugName("OC_UI_ELEMENT", element)
	end
...
-- @tparam Element element The OC UI element object
...
function UIElements.CreateFontString(element, parentFrame)
	local name = private.GetDebugName("OC_FONT_STRING", element)
...
function UIElements.CreateTexture(element, parentFrame, layer, subLayer)
	local name = private.GetDebugName("OC_TEXTURE", element)
	private.debugNameElementLookup[name] = element
