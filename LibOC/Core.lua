...
---Creates a new OC module.
---@param path string The name of the module
---@return table
function OC.Init(path)
...
---Returns an existing OC module.
---@generic T
---@param path `T`
---@return T
function OC.Include(path)
...
function OC.ModuleInfoIterator()
	return private.ModuleInfoIterator, nil, 0
end

---Unloads all modules to simulate a logout.
function OC.DebugLogout()
	private.UnloadAll()
end

---Checks if LibOC code has loaded.
---@return boolean
function OC.IsLibOCLoaded()
...
if ADDON_NAME then
	...
	-- Manually register LibOCClass
	local libOCClassModule = OC.Init("LibOCClass")
	local libTable = LibStub("LibOCClass")
	for k, v in pairs(libTable) do
		libOCClassModule[k] = v
	end
end
