...
local Theme = OC.Init("Util.Theme") ---@class Util.Theme
local Environment = OC.Include("Environment")
local FontPaths = OC.Include("Data.FontPaths")
local Color = OC.Include("Util.Color")
local Table = OC.Include("Util.Table")
local FontObject = OC.Include("Util.FontObject")
...
Theme:OnModuleLoad(function()
	...
	Table.SetReadOnly(OC_ITEM_QUALITY_COLORS)
...
function Theme.GetItemQualityColor(quality)
	return OC_ITEM_QUALITY_COLORS[quality]
end
