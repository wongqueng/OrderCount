...
local Publisher = OC.Init("Util.ReactiveClasses.Publisher") ---@class Util.ReactiveClasses.Publisher
...
local EnumType = OC.Include("Util.EnumType")
local ReactivePublisher = OC.Include("LibOCClass").DefineClass("ReactivePublisher") ---@class ReactivePublisher
local private = {
...
function ReactivePublisher:_HandleData(data, optimizeKey)
	...
	while true do
		...
		if stepProfilingNode then
			OCDEV.Profiling.StartNode(stepProfilingNode)
		end
		...
		elseif stepType == STEP.START_PROFILING then
			...
			OCDEV.Profiling.StartNode(profilingPrefix)
		...
		if stepProfilingNode then
			OCDEV.Profiling.EndNode(stepProfilingNode)
		end
		if ignoreValue then
	...
	if profilingPrefix then
		OCDEV.Profiling.EndNode(profilingPrefix)
	end
end
